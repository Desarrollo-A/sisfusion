...
/**---------------------------------------------------------- */
	...
	public function registroStatusContratacion10v2 () {
		$filtroSede = $this->setFilters($this->session->userdata('id_usuario'), $this->session->userdata('id_sede'));
		return $this->db-> query("SELECT l.idLote, cl.id_cliente, l.validacionEnganche, l.firmaRL,
	...
	public function registroStatusContratacion13 () {
		$filtroSede = $this->setFilters($this->session->userdata('id_usuario'), $this->session->userdata('id_sede'));
		return $this->db-> query("SELECT l.idLote, cl.id_cliente, cl.nombre, cl.apellido_paterno, cl.apellido_materno,
		l.nombreLote, l.idStatusContratacion, l.idMovimiento, CONVERT(VARCHAR,l.modificado,120) as modificado, cl.rfc,
	...
	public function registroStatusContratacion15 () {
		$filtroSede = $this->setFilters($this->session->userdata('id_usuario'), $this->session->userdata('id_sede'));
		return $this->db-> query("SELECT l.idLote, cl.id_cliente, cl.nombre, ISNULL(cl.apellido_paterno, '') apellido_paterno, ISNULL(cl.apellido_materno, '') apellido_materno,
		l.nombreLote, l.idStatusContratacion, l.idMovimiento, CONVERT(VARCHAR,l.modificado,120) as modificado, cl.rfc,
		UPPER(CAST(l.comentario AS varchar(MAX))) as comentario, CONVERT(VARCHAR,l.fechaVenc,120) as fechaVenc, l.perfil, res.nombreResidencial, cond.nombre as nombreCondominio,
		l.ubicacion, ISNULL(tv.tipo_venta, 'Sin especificar') tipo_venta, l.firmaRL, l.validacionEnganche,
		concat(asesor.nombre,' ', asesor.apellido_paterno, ' ', asesor.apellido_materno) as asesor,
		concat(coordinador.nombre,' ', coordinador.apellido_paterno, ' ', coordinador.apellido_materno) as coordinador,
		concat(gerente.nombre,' ', gerente.apellido_paterno, ' ', gerente.apellido_materno) as gerente,
		cond.idCondominio, l.observacionContratoUrgente as vl, se.nombre as nombreSede, ISNULL(oxc0.nombre, 'Normal') tipo_proceso, res.idResidencial
		FROM lotes l
		INNER JOIN clientes cl ON cl.id_cliente = l.idCliente AND cl.idLote = l.idLote
		INNER JOIN condominios cond ON l.idCondominio=cond.idCondominio
		INNER JOIN residenciales res ON cond.idResidencial = res.idResidencial
		...
		concat(coordinador.nombre,' ', coordinador.apellido_paterno, ' ', coordinador.apellido_materno),
		concat(gerente.nombre,' ', gerente.apellido_paterno, ' ', gerente.apellido_materno),
		cond.idCondominio, l.observacionContratoUrgente, se.nombre, ISNULL(oxc0.nombre, 'Normal'), res.idResidencial
		ORDER BY l.nombreLote")->result();
	}
	...
	public function val_ub($idLote){
        ...
        $this->db->where("idLote",$idLote);
		$this->db->where_in('ubicacion', array('1', '2', '4', '5', '3', '15'));
		$query = $this->db->get('lotes');
    ...
    public function getInformation($beginDate, $endDate) {
        ...
        cl.id_cliente_reubicacion, ISNULL(CONVERT(varchar, cl.fechaAlta, 20), '') fechaAlta, ISNULL(oxc0.nombre, 'Normal') tipo_proceso
	...
	public function getLineaVenta($idCliente,$banderaVC){
		...
		return array("clientes" => $clientes, "compartidas" => $compartidas);
	}
