...
/**---------------------------------------------------------- */
    ...
    public function getAllDsByLote($typeTransaction, $fechaInicio, $fechaFinal) {
        ...
        $query = $this->db->query("	SELECT cl.id_cliente, id_asesor, id_coordinador, id_gerente, cl.id_sede, cl.nombre, cl.apellido_paterno,
			...
			INNER JOIN clientes AS cl ON ds.id_cliente = cl.id_cliente AND cl.id_asesor IN (2541, 2562, 2583, 2551, 2572, 2593, 2591, 2570, 2549)
			...
			WHERE idStatusContratacion IN (1, 2, 3) AND idMovimiento IN (31, 85, 20, 63, 73, 82, 92, 96, 99, 102, 104, 107, 108, 109, 111) AND cl.status = 1 
			$filter
    ...
    public function getLotesAllAssistant($idCondominio) {
		...
		WHERE l.status = 1 AND l.idStatusContratacion IN (1, 2, 3) AND l.idMovimiento IN (31, 85, 20, 63, 73, 82, 92, 96, 99, 102, 104, 107, 108, 109, 111) 
        ...
        WHERE l.status = 1 AND l.idStatusContratacion IN (1, 2, 3) AND l.idMovimiento IN (31, 85, 20, 63, 73, 82, 92, 96, 99, 102, 104, 107, 108, 109, 111) AND c.status = 1 AND l.idCondominio = $idCondominio");
        return $query->result_array();
    }
    ...
    public function getLotesTwo($idCondominio) {
		...
		AND lo.idMovimiento IN (31, 85, 20, 63, 73, 82, 92, 96, 99, 102, 104, 107, 108, 109, 111)");
        return $query->result_array();
    }
    ...
    public function setFilters($id_usuario, $id_sede) {
        if(in_array($id_usuario, array(2749, 2807, 2754, 6390, 9775, 2815, 12377, 2799, 10088, 2827, 6012, 12931, 13053, 2875, 14342, 14481, 12276, 16679, 17043, 5342)) || $this->session->userdata('id_rol') == 63) // MJ: VE TODO: CI - ARIADNA MARTINEZ MARTINEZ - MARIELA SANCHEZ SANCHEZ
			$filtroSede = "";
    ...
    public function get_catalogox_contraloria(){
        ...
        return $this->db->query("SELECT CAST(id_catalogo AS varchar(2)) AS id_catalogo, id_opcion, nombre 
        FROM opcs_x_cats 
