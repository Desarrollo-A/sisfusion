...
/*---------------------------------------------------------------------------------------------------------------------------*/
    ...
    //                                     WHERE c.estatus_vigencia = 1 AND c.id_sede IN('".$this->session->userdata('id_sede')."') AND c.lugar_prospeccion = 6 AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
    //             break;
    //         case '19': // SUBDIRECTOR MKTD
    //             return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
    //                                     CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
        ...
        switch ($this->session->userdata('id_rol')) {
            case '20': //GERENTE MKTD
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre,
                                        ...
                                        CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                                        CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente, 
                                        c.fecha_creacion, c.fecha_vencimiento, c.fecha_modificacion, c.estatus_particular, c.estatus, c.otro_lugar
                                        FROM prospectos c 
                                        LEFT JOIN usuarios u ON u.id_usuario = c.id_asesor
                                        LEFT JOIN usuarios us ON us.id_usuario = c.id_coordinador
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
        ...
        switch ($this->session->userdata('id_rol')) {
            ...
            case '5': // ASISTENTE SUBDIRECTOR
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 $filter AND c.id_sede IN(".$this->session->userdata('id_sede').") AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
                break;
            case '3': // GERENTE
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 $filter AND (c.id_asesor = ".$this->session->userdata('id_usuario')." OR c.id_coordinador = ".$this->session->userdata('id_usuario')." OR c.id_gerente = ".$this->session->userdata('id_usuario').") ORDER BY c.fecha_creacion DESC");
                break;
            case '6': // ASISTENTE GERENTE
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 $filter AND c.id_gerente = ".$this->session->userdata('id_lider')." AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
                break;
            case '9': // COORDINADOR
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                ...
                if($this->session->userdata('id_usuario') == 6578 || $this->session->userdata('id_usuario') == 9942 || $this->session->userdata('id_usuario') == 9911) {
                   ...
                   // echo $in;
                    ...
                    return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 $filter AND c.id_asesor = ".$this->session->userdata('id_usuario')." AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
                }
                break;
            case '22': // EJECUTIVO CLUB MADERAS
            case '35': // ATENCIÓN A CLIENTES CLUB MADERAS
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 $filter AND c.tipo = 0 AND c.id_coordinador = ".$this->session->userdata('id_usuario')." ORDER BY c.fecha_creacion DESC");
                break;
                case '23': // SUBDIRECTOR CLUB MADERAS
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.tipo = 0  AND c.estatus_vigencia = 1 $filter AND c.lugar_prospeccion = 12 ORDER BY c.fecha_creacion DESC");
                break;
            case '1': // DIRECTOR
            case '4': // ASISTENTE DIRECTOR
            default: // VE TODOS LOS REGISTROS
    ...
    function getClientsList(){
        switch ($this->session->userdata('id_rol')) {
            case '2': // SUBDIRECTOR
            case '5': // ASISTENTE SUBDIRECTOR
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                                        CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente, 
                                        c.fecha_creacion, c.becameClient fecha_cliente, c.estatus, c.id_asesor, c.lugar_prospeccion, oxc.nombre lp
                                        FROM prospectos c
                                        INNER JOIN usuarios u ON u.id_usuario = c.id_asesor
                                        INNER JOIN usuarios us ON us.id_usuario = c.id_coordinador
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.id_sede IN(".$this->session->userdata('id_sede').") AND c.tipo = 1 ORDER BY c.fecha_creacion DESC");
                break;
            case '3': // GERENTE
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.id_gerente = ".$this->session->userdata('id_usuario')." AND c.tipo = 1 ORDER BY c.fecha_creacion DESC");
                break;
            case '6': // ASISTENTE GERENTE
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.id_gerente = ".$this->session->userdata('id_lider')." AND c.tipo = 1 ORDER BY c.fecha_creacion DESC");
                break;
            case '9': // COORDINADOR
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE (c.id_asesor = ".$this->session->userdata('id_usuario')." OR c.id_coordinador = ".$this->session->userdata('id_usuario').") AND c.tipo = 1 ORDER BY c.fecha_creacion DESC");
                break;
            case '7': // ASESOR
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.id_asesor = ".$this->session->userdata('id_usuario')." AND c.tipo = 1 ORDER BY c.fecha_creacion DESC");
                break;
            case '22': // EJECUTIVO CLUB MADERAS
            case '35': // ATENCIÓN A CLIENTES CLUB MADERAS
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        ...
                                        CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                                        CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente, 
                                        c.fecha_creacion, c.becameClient fecha_cliente, c.estatus, c.lugar_prospeccion, c.id_asesor, oxc.nombre lp
                                        FROM prospectos c 
                                        INNER JOIN usuarios u ON u.id_usuario = c.id_asesor
                                        INNER JOIN usuarios us ON us.id_usuario = c.id_coordinador
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.id_coordinador = ".$this->session->userdata('id_usuario').") AND c.tipo = 1 AND c.lugar_prospeccion = 12 ORDER BY c.fecha_creacion DESC");
                break;
            case '23': // DIRECTOR CLUB MADERAS
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                                        CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente, 
                                        ...
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE  c.lugar_prospeccion = 12 AND c.tipo = 1 ORDER BY c.fecha_creacion DESC");
                break;
            case '19': // SUBDIRECTOR MKTD
            case '20': // GERENTE MKTD
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        ...
                                        CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                                        CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente, 
                                        c.fecha_creacion, c.becameClient fecha_cliente, c.estatus, c.lugar_prospeccion, c.id_asesor, c.lugar_prospeccion, oxc.nombre lp
                                        FROM prospectos c 
                                        INNER JOIN usuarios u ON u.id_usuario = c.id_asesor
                                        INNER JOIN usuarios us ON us.id_usuario = c.id_coordinador
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.id_sede IN(".$this->session->userdata('id_sede').") AND c.tipo = 1 AND c.lugar_prospeccion = 6 ORDER BY c.fecha_creacion DESC");
                break;
            case '1': // DIRECTOR
            case '4': // ASISTENTE DIRECTOR
            default: // VE TODOS LOS REGISTROS
                return $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.correo, c.telefono,
                                        ...
                                        INNER JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        INNER JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.tipo = 1 AND c.lugar_prospeccion != 6 ORDER BY c.fecha_creacion DESC");
                break;
             case '26': // MERCADOLÓGO
                return $this->db->query("SELECT CONCAT(c.nombre,  ' ', c.apellido_paterno, ' ', c.apellido_materno) nombreCliente,
										c.fechaApartado, oxc.nombre lugar_prospeccion, c.telefono1,
										CONCAT(u.nombre,  ' ', u.apellido_paterno, ' ', u.apellido_materno) nombreAsesor,
										r.nombreResidencial, co.nombre nombreCondominio, l.nombreLote, s.nombre ubicacion, c.id_cliente, c.idLote
										FROM clientes c 
										LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
										LEFT JOIN usuarios u ON u.id_usuario = c.id_asesor
										INNER JOIN lotes l ON l.idLote = c.idLote AND l.status = 1
										INNER JOIN condominios co ON co.idCondominio = l.idCondominio
										INNER JOIN residenciales r ON r.idResidencial = co.idResidencial
										LEFT JOIN sedes s ON s.id_sede = l.ubicacion
										WHERE c.status = 1 ORDER BY r.nombreResidencial, co.nombre, l.nombreLote, nombreCliente;");
                break;
        }
    ...
    function getInformationToPrint($id_prospecto){
        return $this->db->query("SELECT p.id_prospecto, p.id_asesor, p.id_coordinador, p.id_gerente, CONCAT(p.nombre, ' ', p.apellido_paterno, ' ', p.apellido_materno) nombre, 
        oxc.nombre personalidad_juridica, p.rfc, p.curp, p.correo, p.telefono, p.telefono_2, p.observaciones, oxc2.nombre lugar_prospeccion, 
        P.otro_lugar, oxc3.nombre plaza_venta, oxc4.nombre nacionalidad,
        CONCAT(us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) asesor, us.telefono tel_asesor,
        CONCAT(us2.nombre, ' ', us2.apellido_paterno, ' ', us2.apellido_materno) coordinador, us2.telefono tel_coordinador,
        CONCAT(us3.nombre, ' ', us3.apellido_paterno, ' ', us3.apellido_materno) gerente, us3.telefono tel_gerente
        FROM prospectos p 
    ...
    {
        return $this->db->query("SELECT id_prospecto, CONCAT(nombre, ' ', apellido_paterno, ' ', apellido_materno) AS cliente, curp, rfc, telefono, telefono_2, correo, personalidad, nacionalidades.nacionalidad, 
...
//    }
    ...
    {
        $this->db->select("*");
        $this->db->where('id_rol', 9);
        $this->db->where('estatus', 1);
        $this->db->where('id_lider', $id_gerente);
        $query = $this->db->get('usuarios');
        return $query->result();
    }
    ...
    function getAsesorByCoords($id_coords)
    {
        ...
        //$this->db->select("*");
        //$this->db->where('id_rol', 7);
        //$this->db->where('estatus', 1);
        //$this->db->where('id_lider', $id_coords);
        //$query = $this->db->get('usuarios');
        return $query->result();
    }
    ...
    /*function getProspectsListByGerente($id_gerente)
    {
        switch ($this->session->userdata('id_rol')) {
            case '19': // SUBDIRECTOR MKTD
                 $query = $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                ...
                LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                WHERE c.estatus_vigencia = 1 AND c.id_sede IN($id_gerente) AND c.lugar_prospeccion = 6 AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
            break;
            ...
            default:
                ...
                LEFT JOIN usuarios us ON us.id_usuario = c.id_coordinador
                LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND
                WHERE c.estatus_vigencia = 1 AND c.id_gerente = ".$id_gerente." AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
            break;
        }
    ...
    function getProspectsListByGerente($id_gerente, $typeTransaction, $beginDate, $endDate, $where)
    {
        ...
        switch ($this->session->userdata('id_rol')) {
            case '19': // SUBDIRECTOR MKTD
                ...
                LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                WHERE c.estatus_vigencia = 1 AND c.id_sede IN($id_gerente) AND c.lugar_prospeccion = 6 AND c.tipo = 0 ".$filter." ORDER BY c.fecha_creacion DESC");
            break;
   ...
   /* function getProspectsListByCoord($id_coord)
    {
    	switch ($this->session->userdata('id_rol')) {
            case '19': // SUBDIRECTOR MKTD
                $query = $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 AND c.id_sede IN($id_coord) AND c.lugar_prospeccion = 6 AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
            break;
            default:
                $query = $this->db->query("SELECT c.id_prospecto, c.nombre, c.apellido_paterno, c.apellido_materno, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 AND (c.id_coordinador = ".$id_coord.") AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
            break;
        }
        ...
        switch ($this->session->userdata('id_rol')) {
            case '19': // SUBDIRECTOR MKTD
                $query = $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 AND c.id_sede IN($id_coord) AND c.lugar_prospeccion = 6 AND c.tipo = 0  $filter ORDER BY c.fecha_creacion DESC");
            break;
            default:
                $query = $this->db->query("SELECT c.id_prospecto, c.nombre, c.apellido_paterno, c.apellido_materno, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 AND (c.id_coordinador = ".$id_coord.") AND c.tipo = 0 ".$filter."  ORDER BY c.fecha_creacion DESC");
            break;
        }
        ...
        switch ($this->session->userdata('id_rol')) {
            ...
            default:
            	$query = $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        ...
                                        CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                                        CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente, 
                                        c.fecha_creacion, c.fecha_vencimiento, c.estatus, c.lugar_prospeccion, oxc.nombre nombre_lp, c.estatus_particular
                                        FROM prospectos c 
                                        LEFT JOIN usuarios u ON u.id_usuario = c.id_asesor
                                        LEFT JOIN usuarios us ON us.id_usuario = c.id_coordinador
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 AND (c.id_asesor = ".$id_asesor.") AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
            break;
            case '28': // SUBDIRECTOR
            	$query = $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                                        CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente, 
                                        c.fecha_creacion, c.fecha_vencimiento, c.estatus, c.lugar_prospeccion, oxc.nombre nombre_lp, c.estatus_particular, c.otro_lugar
                                        FROM prospectos c 
                                        LEFT JOIN usuarios u ON u.id_usuario = c.id_asesor
                                        LEFT JOIN usuarios us ON us.id_usuario = c.id_coordinador
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.lugar_prospeccion = 6 AND c.estatus_vigencia = 1 AND (c.id_asesor = ".$id_asesor.") AND c.tipo = 0 ORDER BY c.fecha_creacion DESC");
            break;
        }
        return $query->result();
    }*/
        ...
        switch ($this->session->userdata('id_rol')) {
            ...
            default:
                $query = $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.estatus_vigencia = 1 AND (c.id_asesor = ".$id_asesor.") AND c.tipo = 0 ".$filter. "ORDER BY c.fecha_creacion DESC");
            break;
            case '28': // SUBDIRECTOR
                $query = $this->db->query("SELECT c.id_prospecto, CONCAT (c.nombre, ' ', c.apellido_paterno, ' ', c.apellido_materno) nombre, c.vigencia,
                                        CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                                        ...
                                        LEFT JOIN usuarios uss ON uss.id_usuario = c.id_gerente
                                        LEFT JOIN opcs_x_cats oxc ON oxc.id_opcion = c.lugar_prospeccion AND oxc.id_catalogo = 9
                                        WHERE c.lugar_prospeccion = 6 AND c.estatus_vigencia = 1 AND (c.id_asesor = ".$id_asesor.") AND c.tipo = 0 ".$filter."  ORDER BY c.fecha_creacion DESC");
            break;
        }
        return $query->result();
    }
    ...
    function getProspectsListBySubdirector($idSubdir, $typeTransaction, $beginDate, $endDate, $where)
    {
        ...
        switch ($this->session->userdata('id_rol')) {
            case '19': // SUBDIRECTOR MKTD
                 $query = $this->db->query("SELECT p.id_prospecto, CONCAT (p.nombre, ' ', p.apellido_paterno, ' ', p.apellido_materno) nombre, p.vigencia,
                CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor,
                CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador,
                CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) gerente,
                p.fecha_creacion, p.fecha_vencimiento, p.estatus, p.estatus_particular, p.lugar_prospeccion, oxp.nombre nombre_lp
                FROM prospectos c
                LEFT JOIN usuarios u ON u.id_usuario = p.id_asesor
                LEFT JOIN usuarios us ON us.id_usuario = p.id_coordinador
                LEFT JOIN usuarios uss ON uss.id_usuario = p.id_gerente
                LEFT JOIN opcs_x_cats oxc ON oxp.id_opcion = p.lugar_prospeccion AND oxp.id_catalogo = 9
                WHERE p.estatus_vigencia = 1 AND p.id_sede IN(AND p.id_sede IN(SELECT idSede FROM roles_x_usuario WHERE idUsuario = $idSubdir AND idRol = 2)) AND p.lugar_prospeccion = 6 AND p.tipo = 0 ".$filter." ORDER BY p.fecha_creacion DESC");
            break;

            default:
                 $query = $this->db->query("SELECT p.id_prospecto, p.nombre, p.apellido_paterno, p.apellido_materno, p.vigencia, 
                 CONCAT (u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) asesor, 
                 CONCAT (us.nombre, ' ', us.apellido_paterno, ' ', us.apellido_materno) coordinador, 
                 CONCAT (uss.nombre, ' ', uss.apellido_paterno, ' ', uss.apellido_materno) 
                 gerente, p.fecha_creacion, p.fecha_vencimiento, p.estatus, p.estatus_particular, p.lugar_prospeccion, oxc.nombre nombre_lp , p.id_sede
                 FROM prospectos p
                 LEFT JOIN usuarios u ON u.id_usuario = p.id_asesor 
                 LEFT JOIN usuarios us ON us.id_usuario = p.id_coordinador 
                 LEFT JOIN usuarios uss ON uss.id_usuario = p.id_gerente 
