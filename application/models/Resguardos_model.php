...
class Resguardos_model extends CI_Model {
    ...
    function getListaRetiros($id) {
        $query = $this->db->query("SELECT r.*, CONVERT(VARCHAR, r.fecha_creacion, 120) AS fecha_creacion ,CONCAT(u.nombre, ' ',u.apellido_paterno, ' ', u.apellido_materno) usuario  from historial_retiros r inner join usuarios u on u.id_usuario=r.id_usuario where id_retiro=$id");
        return $query;
    }
