       ...
       //  $respuesta =  $this->db->query("UPDATE comisiones set comision_total=$comision_total,porcentaje_decimal=$porcentaje,modificado_por='".$this->session->userdata('id_usuario')."' $q WHERE id_comision=$id_comision AND id_lote=$id_lote and id_usuario=$id_usuario");
        ...
        function UpdateVcUser($usuarioOld,$newColab,$rolSelect,$idLote,$idCliente,$comentario,$cuantos){
            ...
            $fecha_actual = strtotime(date("d-m-Y H:i:00"));
                ...
                $response = $this->db->query("insert into ventas_compartidas values($id_cliente,$id_asesor,$coor,$ger,1,GETDATE(),".$this->session->userdata('id_usuario').",$sub, GETDATE(),1,0,0,0);");
            ...
            for($i=0;$i<4;$i++){
                ...
                if(empty($validate->row()->id_usuario)){
                    ...
                    $response = $this->db->query("INSERT INTO comisiones(id_lote,id_usuario,comision_total,estatus,observaciones,ooam,loteReubicado,creado_por,fecha_creacion,porcentaje_decimal,fecha_autorizacion,rol_generado,descuento,idCliente,modificado_por) 
