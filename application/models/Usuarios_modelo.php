...
class Usuarios_modelo extends CI_Model
{
    ...
    public function getChangeLogUsers($id_usuario)
    {
        ...
        $query =  $this->db->query("SELECT CONVERT(VARCHAR,fecha_creacion,20) AS fec_creacion, creador, col_afect,(
                                CASE 
                                    WHEN col_afect = 'usuario' THEN (SELECT UPPER(CONCAT( apellido_paterno,' ',apellido_materno,' ',nombre)) as nombre FROM usuarios WHERE id_usuario = nuevo)
                                    WHEN col_afect = 'id_lider' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = nuevo AND id_catalogo = 10)
                                    WHEN col_afect = 'forma_pago' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = nuevo AND id_catalogo = 16)
                                    WHEN col_afect = 'id_sede' THEN (SELECT UPPER(nombre) AS nombre  FROM sedes WHERE CAST(id_sede AS VARCHAR(45)) = CAST(nuevo AS VARCHAR(45)))
                                    WHEN col_afect = 'id_rol' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = nuevo AND id_catalogo = 1)
                                    WHEN col_afect = 'estatus' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = nuevo AND id_catalogo = 3)
                                    ELSE nuevo  
                                END) AS nuevo,(
                                CASE 
                                    WHEN col_afect = 'usuario' THEN (SELECT UPPER(CONCAT( apellido_paterno,' ',apellido_materno,' ',nombre)) as nombre FROM usuarios WHERE id_usuario = anterior)
                                    WHEN col_afect = 'id_lider' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = anterior AND id_catalogo = 10)
                                    WHEN col_afect = 'forma_pago' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = anterior AND id_catalogo = 16)
                                    WHEN col_afect = 'id_sede' THEN (SELECT UPPER(nombre) AS nombre  FROM sedes WHERE CAST(id_sede AS VARCHAR(45)) = CAST(anterior AS VARCHAR(45)))
                                    WHEN col_afect = 'id_rol' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = anterior AND id_catalogo = 1)
                                    WHEN col_afect = 'estatus' THEN (SELECT UPPER(nombre) AS nombre  FROM opcs_x_cats WHERE id_opcion = anterior AND id_catalogo = 3)
