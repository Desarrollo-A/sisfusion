...
class Reestructura_model extends CI_Model
{
    ...
    public function getListaClientesReubicar() {
        ...
        LEFT JOIN (SELECT idLote, COUNT(*) AS totalContratoNumero FROM propuestas_x_lote GROUP BY idLote) pxl4 ON pxl4.idLote = lo.idLote
    ...
    public function get_valor_lote($id_proyecto, $bandera){
        ini_set('memory_limit', -1);
        $filtro = '';
        $filtroTipo = '';

        if($bandera == 0){
            $filtroTipo = 'proyectoReubicacion';
        }else{
            $filtroTipo = 'idProyecto';
        }

        if($id_proyecto != 0){
            $filtro = "AND lotx.$filtroTipo = $id_proyecto";
        }
        ...
        INNER JOIN (select distinct($filtroTipo) as $filtroTipo from loteXReubicacion) lotx ON lotx.$filtroTipo = con.idResidencial $filtro
    ...
    public function getLotes($id_proyecto){
        ...
        if($id_proyecto != 0){
            $filtro = "AND lotx.idProyecto IN ($id_proyecto)";
        }
        ...
        INNER JOIN condominios con ON con.idCondominio = lot.idCondominio
        INNER JOIN residenciales res ON res.idResidencial = con.idResidencial
        INNER JOIN (SELECT distinct(idProyecto) from loteXReubicacion) lotx ON lotx.idProyecto = con.idResidencial $filtro
    ...
    public function getListaAsignacionCartera() {
        ...
        co.tipo_lote, oxc.nombre nombreTipoLote, ISNULL(u6.id_usuario, 0) idAsesorAsignado, oxc1.nombre estatusPreproceso, lo.estatus_preproceso id_estatus_preproceso
        FROM lotes lo
        INNER JOIN clientes cl ON cl.id_cliente = lo.idCliente AND cl.idLote = lo.idLote AND cl.status = 1 AND cl.proceso NOT IN (2, 3, 4)
        INNER JOIN condominios co ON lo.idCondominio = co.idCondominio
        INNER JOIN residenciales re ON co.idResidencial = re.idResidencial
