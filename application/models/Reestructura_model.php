...
class Reestructura_model extends CI_Model
{
    ...
    public function getReporteVentas()
    {
        ...
        return $this->db->query(
            ...
            SELECT tipoV = 1, cl.id_cliente, lo.tipo_estatus_regreso, oxc0.nombre tipoProceso, UPPER(CAST(re.descripcion AS VARCHAR(150))) nombreResidencial, co.nombre nombreCondominio, lo.nombreLote, 
            lo.sup, CAST(lo.referencia AS VARCHAR(150)) referenciaDestino, lo.idLote, UPPER(CONCAT(cl.nombre, ' ', cl.apellido_paterno, ' ', cl.apellido_materno)) nombreCliente, 
            CASE WHEN u0.id_usuario IS NULL THEN 'SIN ESPECIFICAR' ELSE UPPER(CONCAT(u0.nombre, ' ', u0.apellido_paterno, ' ', u0.apellido_materno)) END nombreAsesor, 
            CASE WHEN u2.id_usuario IS NULL THEN 'SIN ESPECIFICAR' ELSE UPPER(CONCAT(u2.nombre, ' ', u2.apellido_paterno, ' ', u2.apellido_materno)) END nombreGerente, 
            CASE WHEN u3.id_usuario IS NULL THEN 'SIN ESPECIFICAR' ELSE UPPER(CONCAT(u3.nombre, ' ', u3.apellido_paterno, ' ', u3.apellido_materno)) END nombreSubdirector, 
            cl.fechaApartado, sl.nombre estatusLote, sc.nombreStatus estatusContratacion, mo.descripcion detalleUltimoEstatus, 
            CASE WHEN oxc0.id_opcion IN (2, 3, 4) THEN CAST(ISNULL(lo2.nombreLote, lo.nombreLote) AS VARCHAR(20)) END AS nombrePvOrigen, 
            CASE WHEN oxc0.id_opcion IN (2, 3, 4) THEN CAST(ISNULL(lo2.idLote, lo.idLote) AS VARCHAR(20)) END AS idLotesOrigen, CAST(lo2.sup AS VARCHAR) supLoteOrigen, 
            CAST(lo2.referencia as varchar(150)) as referenciaOrigen, lo2.sup supSumLoteOrigen, 
