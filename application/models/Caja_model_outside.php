<?php class Caja_model_outside extends CI_Model {
    ...
    public function __construct() {
        ...
        public function getResidencialDis2($rol) {
            ...
            --WHERE re.idResidencial NOT IN (14)
        ...
        public function getResidencial() {
            ...
            FROM residenciales
            WHERE status = 1 --AND idResidencial NOT IN (14)
    ...
    public function aplicaLiberacion($datos) {
        ...
        {
            return $this->db->query("SELECT u0.id_rol, u0.id_usuario id_asesor, 
            CASE WHEN u0.id_rol = 9 THEN 0 ELSE u0.id_lider END id_coordinador, 
            CASE WHEN u0.id_rol = 9 THEN u1.id_usuario ELSE u2.id_usuario END id_gerente, 
            CONCAT(u0.nombre, ' ', u0.apellido_paterno, ' ', u0.apellido_materno) nombre 
            FROM usuarios u0
            LEFT JOIN usuarios u1 ON u1.id_usuario = u0.id_lider AND u1.estatus = 1
            LEFT JOIN usuarios u2 ON u2.id_usuario = u1.id_lider AND u2.estatus = 1
            WHERE u0.id_rol IN (7, 9) AND u0.estatus = 1 AND ISNULL(u0.correo, '') NOT LIKE '%SINCO%' AND ISNULL(u0.correo, '') NOT LIKE '%test_%' AND u0.id_usuario NOT IN  (4415, 11053, 11160, 11161, 11179, 11750, 12187, 12874)")->result();
        }
...
function updateReference($data, $id_referencia) {
    ...
    public function getNameLote($idLote) {
        return $this->db-> query("SELECT nombreLote, tipo_venta from lotes where idLote =  $idLote")->row(); 
    }
...
public function infoBloqueos($idLote){
    ...
    public function getGerente(){
            $query = $this->db-> query("SELECT id_usuario, CONCAT(id_usuario,' - ',nombre, ' ', apellido_paterno, ' ', apellido_materno) nombre FROM usuarios WHERE id_rol = 3 AND estatus = 1 OR id_usuario IN (6482, 666, 5, 7092, 14161)");
            return $query->result_array();
    }
    ...
    public function getCoordinador($id_gerente){
           ...
           $query = $this->db-> query("SELECT id_usuario, CONCAT(id_usuario,' - ',nombre, ' ', apellido_paterno, ' ', apellido_materno) nombre FROM usuarios 
            WHERE (id_rol IN (7, 9, 3) AND (rfc NOT LIKE '%TSTDD%' AND ISNULL(correo, '' ) NOT LIKE '%test_%') AND estatus = 1) OR (id_usuario IN (2567, 4064, 4068, 2588, 4065, 4069, 2541, 2583, 2562, 2572,2559,2576, 2595, 2570, 1383, 5, 7092, 10807)) ORDER BY nombre");
...
public function insert_vcompartidas($data){
    ...
    public function data_cliente(){
        ...
        $query = $this->db->query("select cl.id_cliente ,id_asesor ,id_coordinador ,id_gerente ,cl.id_sede ,cl.nombre ,cl.apellido_paterno, lotes.referencia,
                                ...
                                ,engancheCliente ,concepto ,fechaEnganche ,cl.idTipoPago ,expediente ,cl.status ,cl.idLote ,fechaApartado, cl.fechaVencimiento , cl.usuario, cond.idCondominio, cl.fecha_creacion, cl.creado_por, 
 ...
 //////////////////////////// CASAS /////////////////////////////////////
    ...
    public function allUserVentas()
    {
		...
		(CASE u1.id_rol WHEN 9 THEN UPPER(CONCAT(u1.nombre, ' ', u1.apellido_paterno, ' ', u1.apellido_materno)) WHEN 3 THEN UPPER(CONCAT(u1.nombre, ' ', u1.apellido_paterno, ' ', u1.apellido_materno)) END) coordinador,
		(CASE u1.id_rol WHEN 3 THEN u1.id_usuario ELSE u2.id_usuario END) id_gerente,
		(CASE u1.id_rol WHEN 3 THEN UPPER(CONCAT(u1.nombre, ' ', u1.apellido_paterno, ' ', u1.apellido_materno)) ELSE UPPER(CONCAT(u2.nombre, ' ', u2.apellido_paterno, ' ', u2.apellido_materno)) END) gerente,
		(CASE u1.id_rol WHEN 3 THEN u1.id_lider ELSE u3.id_usuario END) id_subdirector, 
		(CASE u1.id_rol WHEN 3 THEN UPPER(CONCAT(u2.nombre, ' ', u2.apellido_paterno, ' ', u2.apellido_materno)) ELSE UPPER(CONCAT(u3.nombre, ' ', u3.apellido_paterno, ' ', u3.apellido_materno)) END) subdirector,
...
        WHEN u3.id_usuario = 7886 THEN 5
		...
		(CASE u1.id_rol 
			WHEN 3 THEN (CASE WHEN u2.id_lider = 2 THEN 'NO APLICA' ELSE UPPER(CONCAT(u3.nombre, ' ', u3.apellido_paterno, ' ', u3.apellido_materno)) END) 
			ELSE 
			CASE 
				WHEN u3.id_usuario IN (7092, 9471, 681, 609, 690, 7886, 692) THEN UPPER(CONCAT(u4.nombre, ' ', u4.apellido_paterno, ' ', u4.apellido_materno))
				WHEN (u3.id_usuario = 5 AND u0.id_sede = '11') THEN 'NO APLICA' ELSE 'NO APLICA' 
			END 
		END) regional,
		CASE 
		WHEN (u0.id_sede = '13' AND u3.id_lider = 7092) THEN 3
		WHEN (u0.id_sede = '13' AND u3.id_lider = 3) THEN 7092
		ELSE 0 END id_regional_2,
		CASE 
		WHEN (u0.id_sede = '13' AND u3.id_lider = 7092) THEN UPPER(CONCAT(u5.nombre, ' ', u5.apellido_paterno, ' ', u5.apellido_materno))
		WHEN (u0.id_sede = '13' AND u3.id_lider = 3) THEN UPPER(CONCAT(u4.nombre, ' ', u4.apellido_paterno, ' ', u4.apellido_materno))
		...
		UNION ALL
		(SELECT id_usuario as id_asesor, 0 id_sede, UPPER(CONCAT(nombre, ' ', apellido_paterno, ' ', apellido_materno)) as asesor, 0 id_coordinador, 'NO APLICA' coordinador, 0 id_gerente, 'NO APLICA' gerente,
