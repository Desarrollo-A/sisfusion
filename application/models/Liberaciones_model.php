...
class Liberaciones_model extends CI_Model {
    ...
    public function getLotesBloqueados($condicion, $filtroLotes) 
    {
        ...
        WHERE lo.idStatusLote = (8) and lo.status = 1 $condicion $filtroLotes
    ...
    public function getLotesEnProcesoBloqueo($condicion) 
    {
        ini_set('memory_limit', -1);
        $query = $this->db->query(
        "WITH proceso_liberacion_temp AS (SELECT *, ROW_NUMBER() OVER(PARTITION BY idLote ORDER BY fecha_creacion DESC) AS rn FROM proceso_liberaciones WHERE estatus = 1)
