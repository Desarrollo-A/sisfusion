...
class Liberaciones_model extends CI_Model {
    ...
    public function getLotesParaLiberacion() 
    {
        ...
        SELECT re.nombreResidencial, co.nombre AS nombreCondominio, lo.nombreLote, lo.idLote, co.idCondominio, re.idResidencial, lo.idCliente,
			...
			CASE WHEN oxc3.nombre IS NOT NULL THEN oxc3.nombre ELSE 'Sin concepto' END nombreConceptoLiberacion, pl.comentario, pl.precioLiberacion, pl.plazo
        FROM lotes AS lo
			...
			LEFT JOIN opcs_x_cats AS oxc ON pl.proceso_lib = oxc.id_opcion AND oxc.id_catalogo = 133
    ...
    public function historialLiberacionLote($idLote) 
    {
        ...
        WHERE lo.tipo_venta = 1 AND cl.status = 1 AND pl.estatus = 1 AND lo.idStatusContratacion IN (9,10,13,14,15)
            AND pl.idLote= ?
