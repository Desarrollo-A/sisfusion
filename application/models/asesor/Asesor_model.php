...
class Asesor_model extends CI_Model {
    ...
    function getinfoCliente($id_cliente) {
        ...
        ds.costom2f, oc4.nombre AS reg_nom, cl.cp_fac, cl.ladaTel1, cl.ladaTel2
         FROM clientes cl  
	...
	function getinfoCopropietario($id_cliente){
		return $this->db->query("SELECT id_copropietario, id_cliente, regimen_matrimonial as regimen_valor, estado_civil as estado_valor, co.nacionalidad as nacionalidad_valor, co.nombre as nombre_cop, apellido_paterno, apellido_materno, telefono, telefono_2, correo, fecha_nacimiento, originario_de, conyuge, domicilio_particular, personalidad_juridica, ocupacion, empresa, posicion,  antiguedad, edadFirma, direccion, tipo_vivienda, rfc, ladaTel, ladaCel
    ...
    public function getDataDs1($id_cliente) { // DATA FROM DEPOSITO_SERIEDAD
        ...
        cl.tipo_comprobanteD, ISNULL(oxc0.nombre, 'Normal') tipo_proceso, cl.proceso
        FROM clientes as cl
    ...
    public function getDataDs2($id_cliente) { // DATA FROM DEPOSITO_SERIEDAD_CONSULTA
        ...
        aut.estatus as estatus, 'NULL' as tipo_comprobanteD, 'Normal' tipo_proceso, 1 proceso
	...
	public function selectDS($cliente) {
        return $this->db->query("SELECT ISNULL(lot.tipo_venta, 0) AS tipo_venta,
                                        ...
                                        lot.nombreLote,
                                        lot.sup,
                                        FORMAT(TRY_CAST(lot.precio AS float), 'C') AS precio,
                                        res.nombreResidencial,
                                        con.nombre AS nombreCondominio,
                                        con.idCondominio,
                                        ds.id AS idDeposito,
                                        ds.clave,
                                        res.idResidencial AS desarrollo,
                                        con.tipo_lote AS tipoLote,
                                        ds.idOficial_pf,
                                        ds.idDomicilio_pf,
                                        ...
                                        cl.nombre_conyuge,
                                        cl.nacionalidad,
                                        cl.originario_de AS originario,
                                        cl.estado_civil,
                                        cl.regimen_matrimonial,
                                        cl.ocupacion,
                                        cl.empresa,
                                        cl.puesto,
                                        cl.antiguedad,
                                        ...
                                        FORMAT(TRY_CAST(ds.importOferta AS float), 'C') AS importOferta,
                                        ds.letraImport,
                                        FORMAT(TRY_CAST(ds.cantidad AS float), 'C') AS cantidad,
                                        ds.letraCantidad,
                                        FORMAT(TRY_CAST(ds.saldoDeposito AS float), 'C') AS saldoDeposito,
                                        FORMAT(TRY_CAST(REPLACE(ds.aportMensualOfer, ',', '') AS decimal(16, 2)), 'C') AS aportMensualOfer,
                                        ds.fecha1erAport,
                                        ds.plazo,
                                        ds.fechaLiquidaDepo,
                                        ds.fecha2daAport,
                                        ...
                                        ds.anio,
                                        ds.observacion,
                                        ds.nombreFirmaAsesor,
                                        ds.fechaCrate,
                                        ds.id_cliente,
                                        lot.referencia,
                                        FORMAT(TRY_CAST(ds.costom2f AS float), 'C') AS costom2f,
                                        cl.lugar_prospeccion,
                                        ds.fecha_modificacion,
                                        ds.costoM2_casas,
                                        cl.descuento_mdb,
                                        tipo_nc,
                                        printPagare,
                                ...
                                INNER JOIN residenciales res ON res.idResidencial = con.idResidencial
                                INNER JOIN deposito_seriedad ds ON ds.id_cliente = cl.id_cliente
                                WHERE cl.id_cliente = $cliente")->result();
    }
...
public function getSupOneTodos(){
	...
	public function get_auts_by_lote($idLote)
	{
		...
		lotes.nombreLote, autorizaciones.estatus, autorizaciones.autorizacion,CONVERT(VARCHAR,autorizaciones.fecha_creacion,20) AS fecha_creacion, 
...
public function insertControversia($data){
	...
	/*modelos de la facturacion en el ds*/
    function getPaises(){
        $query = $this->db->query("SELECT * FROM TPaises;");
        return $query->result_array();
    }
    function getEstados(){
        $query = $this->db->query("SELECT CodigoEstadoSAT, Estado FROM AcCatCodigosPostalesSAT GROUP BY CodigoEstadoSAT, Estado ORDER BY Estado;");
        return $query->result_array();
    }
    function getCP($EDO){
        $query = $this->db->query("SELECT * FROM AcCatCodigosPostalesSAT WHERE CodigoEstadoSAT='".$EDO."'");
        return $query->result_array();
    }
    function getColonia($CP){
        $query = $this->db->query("SELECT *  FROM TColonia
        LEFT JOIN AcCatCodigosPostalesSAT c  ON TColonia.cp = c.CodigoPostalSAT 
        JOIN TMunicipio ON TMunicipio.c_municipio = c.CodigoMunicipioSAT AND TMunicipio.clave_estado = c.CodigoEstadoSAT
        where cp =".$CP);
        return $query->result_array();
    }
    function getTipoPersonalidadJuridica()
    {
        $query = $this->db->query("SELECT * FROM TRegimenFiscal WHERE estatus=1");
        return $query->result_array();
    }
    function getTMunicipioByIdMunicipio($idMunicipio){
        $query = $this->db->query("SELECT * FROM TMunicipio WHERE idMunicipio=".$idMunicipio);
        return $query->result_array();
    }
    ...
    public function revisaOU($idLote){
