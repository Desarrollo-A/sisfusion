...
class Reporte_model extends CI_Model {
    ...
    public function getChartQueries( $generalFilters, $generalFiltersExt, $filtroSt, $id_rol, $beginDate, $endDate, $filtroEsp) {
        ...
        LEFT JOIN (SELECT FORMAT( SUM(CASE WHEN (lo.totalNeto2 IS NULL OR lo.totalNeto2 = 0.00) THEN (CASE WHEN ISNUMERIC(ds.costom2f) = 1 THEN 0 ELSE ISNULL(TRY_CAST(ds.costom2f AS DECIMAL(16, 2)) * lo.sup, lo.precio * lo.sup) END) ELSE lo.totalNeto2 END), 'C') total,
        COUNT(*)
        ...
        $query = $this->db->query("SELECT
            ...
            FROM (
				...
				FROM (
					...
					SUM(CASE WHEN (lo.totalNeto2 IS NULL OR lo.totalNeto2 = 0.00) THEN (CASE WHEN ISNUMERIC(ds.costom2f) = 1 THEN 0 ELSE ISNULL(TRY_CAST(ds.costom2f AS DECIMAL(16, 2)) * lo.sup, lo.precio * lo.sup) END) ELSE lo.totalNeto2 END) total
					FROM clientes cl
					INNER JOIN lotes lo ON lo.idLote = cl.idLote
                    INNER JOIN condominios co ON co.idCondominio = lo.idCondominio
