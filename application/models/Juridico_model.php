...
class Juridico_model extends CI_Model {
	...
	public function registroStatusContratacion7 ($typeTransaction = "", $beginDate = "", $endDate = "", $idResidencial = "",$idCondominio = '') {
        ...
        ORDER BY l.modificado DESC");
			...
			if(in_array($this->session->userdata('id_usuario'), array(2765, 2776, 10463, 2820, 2876, 10437, 5468, 2764, 6856, 2800, 11129, 11258, 12047, 12842, 11097, 2825, 14183, 15046, 11125, 10427, 15025)))
	...
	public function get_users_reassing(){
		...
		WHERE us.id_usuario IN (2776, 10463, 2765, 2820, 2876, 10437, 5468, 2764, 6856, 2800, 11258, 12047, 12842, 11097, 14183, 15046, 11125, 10427, 15025) AND us.id_rol = 15 AND us.estatus = 1 
