...
class Juridico_model extends CI_Model {
	...
	public function registroStatusContratacion7 ($typeTransaction = "", $beginDate = "", $endDate = "", $idResidencial = "",$idCondominio = '') {
	    if($this->session->userdata('id_usuario') == 2762 || $this->session->userdata('id_usuario') == 6096){
			...
			if(in_array($this->session->userdata('id_usuario'), array(2765, 2776, 10463, 2820, 2876, 10437, 15418, 2764, 6856, 11129, 11258, 12047, 15108, 11097, 2825, 14183, 15046, 11125, 10427, 15025, 17118, 11468, 16263, 18006)))
	...
	public function get_users_reassing(){
		...
		WHERE us.id_usuario IN (2776, 10463, 2765, 2820, 2876, 10437, 15418, 2764, 6856, 11258, 12047, 15108, 11097, 14183, 14872, 14873, 15046, 11125, 10427, 15025, 17118, 11468, 15563, 16263, 18006) AND us.id_rol = 15 AND us.estatus = 1 
