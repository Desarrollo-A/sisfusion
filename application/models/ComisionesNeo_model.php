...
class ComisionesNeo_model extends CI_Model {
    ...
    public function UpdateBanderaPagoComision($idLote, $bonificacion, $FechaAplicado, $FPoliza, $Aplicado){
        return $this->db->query("UPDATE pago_comision SET bandera = 0/*(case when numero_dispersion >= 2 then 2 else 0 end)*/, fecha_modificacion = GETDATE(), bonificacion = ".$bonificacion.", fecha_neodata = '".$FechaAplicado."', modificado_por = 'NEO' WHERE id_lote = ".$idLote."");
    ...
    public function UpdateBanderaPagoComisionNewNeo(){
        return $this->db->query("UPDATE pago_comision SET bandera = 55 FROM pago_comision WHERE (((abonado - ultimo_pago) < 1 AND (new_neo-ultimo_pago) < 1) OR (total_comision - abonado) < 1) AND bandera = 0 AND total_comision > 1");
    }
