...
class Comisiones_model extends CI_Model {
    ...
    public function getDataDispersionPago(){
        ...
        mc.opcion as opcionMensualidad, ISNULL(opc_mc.nombre, 'Mensualidad No Existente') AS nombreMensualidad
        FROM lotes l
        INNER JOIN clientes cl ON cl.id_cliente = l.idCliente
        INNER JOIN condominios cond ON l.idCondominio = cond.idCondominio
        INNER JOIN residenciales res ON cond.idResidencial = res.idResidencial
        ...
        LEFT JOIN opcs_x_cats opc_mc ON opc_mc.id_opcion = mc.opcion AND opc_mc.id_catalogo = 126
    ...
    function getDatosHistorialPago($anio,$proyecto,$tipo) {
        ...
        com.porcentaje_decimal, pci1.abono_neodata pago_cliente, pci1.pago_neodata, pci2.abono_pagado pagado, 
        ...
        (CASE WHEN cl.lugar_prospeccion IS NULL THEN 0 ELSE cl.lugar_prospeccion END) lugar_prospeccion, lo.referencia, 
        ...
        LEFT JOIN opcs_x_cats oxc0 ON oxc0.id_opcion = cl.proceso AND oxc0.id_catalogo = 97
        WHERE $filtro_02
        GROUP BY pci1.id_comision,com.ooam,com.loteReubicado, lo.nombreLote, re.nombreResidencial, 
        co.nombre, lo.totalNeto2, com.comision_total, com.porcentaje_decimal, pci1.abono_neodata,
        pci1.pago_neodata, pci2.abono_pagado, pci1.estatus, pci1.fecha_abono, pci1.id_usuario, 
        pci1.id_pago_i, u.nombre, u.apellido_paterno, u.apellido_materno, oprol.nombre, oxcest.nombre, 
        oxcest.id_opcion, pci1.descuento_aplicado, cl.lugar_prospeccion, lo.referencia, com.estatus, 
        pac.bonificacion, u.estatus,pe.id_penalizacion, oxcest.color, cl.estructura, oprol2.nombre, 
        cl.proceso, oxc0.nombre, cl.id_cliente_reubicacion_2 ORDER BY lo.nombreLote");
    }
    ...
    function getDatosNuevasSNL(){
        ...
        pci2.abono_pagado pagado, com.comision_total-pci2.abono_pagado restante, oxc.nombre as lugar_prosp, pci1.estatus, cl.personalidad_juridica, pac.porcentaje_abono 
        ...
        AND oxc.id_catalogo = 30   AND com.rol_generado = 42 AND lo.ubicacion in (1)
    ...
    public function getDatosAbonadoSuma11($idlote,$ooam){
        // validar 
        return $this->db->query("SELECT SUM(pci.abono_neodata) abonado, pac.total_comision, c2.abono_pagado, lo.totalNeto2, cl.lugar_prospeccion,cl.estructura,mc.opcion
        FROM lotes lo
        INNER JOIN clientes cl ON cl.id_cliente = lo.idCliente
        ...
        LEFT JOIN mensualidad_cliente mc ON mc.id_lote = lo.idLote
        WHERE lo.status = 1 AND cl.status = 1 AND c1.estatus = 1 AND lo.idLote in ($idlote)
        GROUP BY lo.idLote, lo.referencia, pac.total_comision, lo.totalNeto2, cl.lugar_prospeccion, c2.abono_pagado,cl.estructura,mc.opcion");
    }
    ...
    function getDatosResguardoContraloria($usuario,$proyecto){
        ...
        FROM pago_comision_ind WHERE (estatus in (11,3) OR descuento_aplicado = 1) 
        GROUP BY id_comision) pci2 ON pci1.id_comision = pci2.id_comision
        INNER JOIN comisiones com ON pci1.id_comision = com.id_comision and com.estatus NOT IN(0)
        INNER JOIN lotes lo ON lo.idLote = com.id_lote  
        INNER JOIN condominios co ON co.idCondominio = lo.idCondominio
        INNER JOIN residenciales re ON re.idResidencial = co.idResidencial
        INNER JOIN usuarios u ON u.id_usuario = com.id_usuario
    ...
    public function getDataLiquidadasPago($val = '') {
        ...
        LEFT JOIN (SELECT COUNT(*) total, id_lote FROM comisiones WHERE ooam = 1 GROUP BY id_lote) ooam ON ooam.id_lote = l.idLote
        LEFT JOIN (SELECT COUNT(*) total, id_lote FROM comisiones WHERE ooam = 2 GROUP BY id_lote) ventas ON ventas.id_lote = l.idLote
        WHERE l.idStatusContratacion IN(9,10,13,14,15) AND cl.status = 1 AND l.status IN (0,1) AND l.registro_comision IN (7) AND l.tipo_venta IS NOT NULL
        ORDER BY l.idLote");
        return $query ;
    }
    ...
    function getDatosNuevasFacturasContraloria($proyecto,$condominio) {
        if( $this->session->userdata('id_rol') == 31) { // INTERNOMEX
            $filtro = "pci1.estatus IN (8, 88) AND com.id_usuario = $condominio";
            $where = "";
        } else { // CONTRALORÍA
    ...
    function getPrestamosXporUsuario(){
        ...
        LEFT JOIN relacion_pagos_prestamo rpp ON rpp.id_prestamo = p.id_prestamo
        LEFT JOIN pago_comision_ind pci ON pci.id_pago_i = rpp.id_pago_i AND pci.estatus in (18,19,20,21,22,23,24,25,26,28,29,30,89) AND pci.descuento_aplicado = 1
        LEFT JOIN opcs_x_cats opc on opc.id_opcion=p.tipo and opc.id_catalogo=23
...
function getDatosGralInternomex(){ 
    return $this->db->query("(SELECT TOP 10 pci1.id_pago_i as id_pago, pci1.id_comision, re.nombreResidencial as proyecto, co.nombre as condominio, lo.nombreLote as lote, lo.referencia, lo.totalNeto2 precio_lote, re.empresa, /*com.comision_total, pci1.pago_neodata, */pci1.abono_neodata pago_cliente, 
    ...
    CONCAT(u.nombre, ' ',u.apellido_paterno, ' ', u.apellido_materno) usuario, CASE WHEN cl.estructura = 1 THEN oprol2.nombre ELSE oprol.nombre END as puesto, pci1.fecha_pago_intmex, oxcfp.nombre forma_pago, sed.nombre, (CASE u.estatus WHEN 0 THEN 'BAJA' ELSE 'ACTIVO' END) estatus_usuario, u.rfc, pci1.estatus
    FROM pago_comision_ind pci1 
    INNER JOIN comisiones com ON pci1.id_comision = com.id_comision
    INNER JOIN lotes lo ON lo.idLote = com.id_lote AND lo.status = 1 
    INNER JOIN condominios co ON co.idCondominio = lo.idCondominio 
    ...
    INNER JOIN pago_comision pac ON pac.id_lote = com.id_lote
    INNER JOIN opcs_x_cats oxcest ON oxcest.id_opcion = pci1.estatus AND oxcest.id_catalogo = 23 
	INNER JOIN sedes sed ON sed.id_sede = (CASE u.id_usuario WHEN 2 THEN 2 WHEN 3 THEN 2 WHEN 26 THEN 2 WHEN 27 THEN 2 WHEN 607 THEN 1 WHEN 1980 THEN 2 WHEN 1981 THEN 2 WHEN 1982 THEN 2 WHEN 1988 THEN 2 WHEN 4 THEN 5 WHEN 5 THEN 3 WHEN 9629 THEN 2 WHEN 13546 THEN 2 WHEN 13547 THEN 2 WHEN 13548 THEN 2 ELSE u.id_sede END) AND sed.estatus = 1
    ...
    u.apellido_materno, oprol.nombre, pci1.fecha_pago_intmex, oxcfp.nombre, u.forma_pago, sed.nombre, u.estatus, u.rfc, cl.estructura, oprol2.nombre, pci1.estatus)
    UNION 
    (SELECT pci1.id_pago_i as id_pago, pci1.id_comision, re.nombreResidencial as proyecto, co.nombre as condominio, lo.nombreLote as lote, lo.referencia, lo.totalNeto2 precio_lote, re.empresa, /*com.comision_total, pci1.pago_neodata, */pci1.abono_neodata pago_cliente, 
    (CASE u.forma_pago WHEN 3 THEN sed.impuesto ELSE 0 END) valimpuesto, 
    ...
    INNER JOIN opcs_x_cats oxcest ON oxcest.id_opcion = pci1.estatus AND oxcest.id_catalogo = 23 
	INNER JOIN sedes sed ON sed.id_sede = (CASE u.id_usuario 
	WHEN 2 THEN 2 WHEN 3 THEN 2 WHEN 26 THEN 2 WHEN 27 THEN 2 WHEN 607 THEN 1 WHEN 1980 THEN 2 WHEN 1981 THEN 2 WHEN 1982 THEN 2 WHEN 1988 THEN 2 WHEN 4 THEN 5 WHEN 5 THEN 3 WHEN 9629 THEN 2 WHEN 13546 THEN 2 WHEN 13547 THEN 2 WHEN 13548 THEN 2 ELSE u.id_sede END) AND sed.estatus = 1
    INNER JOIN opcs_x_cats oxcfp ON oxcfp.id_opcion =  u.forma_pago AND oxcfp.id_catalogo = 16 
    LEFT JOIN clientes cl ON cl.id_cliente = com.idCliente
    LEFT JOIN opcs_x_cats oprol2 ON oprol2.id_opcion = com.rol_generado AND oprol2.id_catalogo = 83
    ...
    function getDatosHistorialPagoEstatus($proyecto, $estado, $usuario,$tipoDeDatos) {
        $filtro_00 = ($proyecto === '0') ? '' : " AND re.idResidencial = $proyecto ";
        $userWhereClause = ($usuario != 0) ? "AND com.id_usuario = $usuario" : '';
        $sqlNombre = '';
        $comisiones = '';
        if ($tipoDeDatos == 1 || $tipoDeDatos == 2 ){
            // tpo de datos  para comer
        $sqlNombre = "(CASE WHEN com.ooam = 2 THEN CONCAT(lo.nombreLote,' <i>(',com.loteReubicado,')</i>') ELSE lo.nombreLote END) nombreLote,";
        $pago_ind = " pago_seguro_ind ";
        $comisiones = "";

        }else if( $tipoDeDatos == 3){
            // casas
            $pago_ind = "";
            $comisiones = "";
        }else if($tipoDeDatos == 4){
            // seguros
            $sqlNombre = "lo.nombreLote AS nombreLote,";
            $pago_ind = " pago_seguro_ind ";
            $comisiones = "";
        }
        ...
        u.forma_pago, pac.porcentaje_abono, 
        (CASE WHEN com.ooam = 1 THEN  CONCAT(oxcest.nombre,' (EEC)') ELSE oxcest.nombre END) estatus_actual, 
        oxcest.id_opcion id_estatus_actual, pci1.descuento_aplicado, 
        (CASE WHEN cl.lugar_prospeccion IS NULL THEN 0 ELSE cl.lugar_prospeccion END) lugar_prospeccion, 
        lo.referencia, com.estatus estado_comision, pac.bonificacion, u.estatus as activo, lo.tipo_venta, oxcest.color, 
        ...
        cl.proceso, 
        ISNULL(cl.id_cliente_reubicacion_2, 0)

        FROM $pago_ind pci1 
        -- $pago_ind = pago_seguro_ind
        LEFT JOIN (SELECT SUM(abono_neodata) abono_pagado, id_comision 
        FROM $pago_ind WHERE (estatus in (11,3) OR descuento_aplicado = 1) 
        --  $pago_ind = pago_seguro_ind   
        GROUP BY id_comision) pci2 ON pci1.id_comision = pci2.id_comision
        INNER JOIN comisiones com ON pci1.id_comision = com.id_comision
        -- $comisiones = comsiones_seguros
        INNER JOIN lotes lo ON lo.idLote = com.id_lote AND lo.status = 1 
        INNER JOIN condominios co ON co.idCondominio = lo.idCondominio
        ...
        INNER JOIN opcs_x_cats oxcest ON oxcest.id_opcion = pci1.estatus AND oxcest.id_catalogo = 23
        LEFT JOIN penalizaciones pe ON pe.id_lote = lo.idLote AND pe.id_cliente = lo.idCliente 
        --  penalizacion  se va con el comision
        LEFT JOIN opcs_x_cats oprol2 ON oprol2.id_opcion = com.rol_generado AND oprol2.id_catalogo = 83
        ...
        co.nombre, lo.totalNeto2, com.comision_total, com.porcentaje_decimal, pci1.abono_neodata, 
        pci1.pago_neodata, pci2.abono_pagado, pci1.estatus, pci1.fecha_abono, pci1.id_usuario, 
        u.forma_pago, pci1.id_pago_i, pac.porcentaje_abono, u.nombre, u.apellido_paterno,u.apellido_materno, 
        oprol.nombre, oxcest.nombre, oxcest.id_opcion, pci1.descuento_aplicado, lo.referencia, 
        com.estatus, pac.bonificacion, u.estatus, lo.tipo_venta, oxcest.color, pe.id_penalizacion, 
        cl.lugar_prospeccion, com.estatus, cl.estructura, oprol2.nombre, cl.proceso, oxc0.nombre, id_cliente_reubicacion_2 ");
    }
    ...
    public function porcentajes($clienteData, $totalNeto, $plan_comision,$ooamDispersion){
        ...
        if($plan_comision == 64 ||$plan_comision == 65 ||$plan_comision == 66 || $plan_comision == 84 ||$plan_comision == 85 ||$plan_comision == 86 ){
            ...
            $joinLotes = $ooamDispersion == 2 ? "INNER JOIN lotes lo ON lo.idLote = cA.idLote" : 'INNER JOIN lotes lo ON lo.idCliente = cA.id_cliente';
        }
        ...
        IF @numAsesores > 1
            ...
            (($totalNeto/100)*(pl.comRe/ $numeroRegionales))*((SELECT COUNT(id_gerente) FROM clientes cD WHERE cD.status = 1 AND cD.id_regional = u1.id_usuario and cD.id_cliente = @idCliente)+(SELECT COUNT(id_gerente) FROM ventas_compartidas vD WHERE vD.estatus = 1 AND vD.id_regional = u1.id_usuario AND vD.id_cliente = @idCliente)) comision_total, 
            (pl.neoRe/ $numeroRegionales)*((SELECT COUNT(id_gerente) FROM clientes cD WHERE cD.status = 1 AND cD.id_regional = u1.id_usuario and cD.id_cliente = @idCliente)+(SELECT COUNT(id_gerente) FROM ventas_compartidas vD WHERE vD.estatus = 1 AND vD.id_regional = u1.id_usuario AND vD.id_cliente = @idCliente)) porcentaje_neodata, 
    ...
    public function getDetailPrestamo($idPrestamo)
    {
        ...
        CASE WHEN pa.estatus=1 THEN 'Activo' WHEN pa.estatus=2 THEN 'Liquidado' WHEN pa.estatus=3 THEN 'Liquidado' END AS estatus,sed.nombre as sede
                ...
                INNER JOIN pago_comision_ind pci ON pci.id_pago_i = rpp.id_pago_i AND pci.estatus IN(18,19,20,21,22,23,24,25,26,28,29,30,89) AND pci.descuento_aplicado = 1
                INNER JOIN comisiones c ON c.id_comision = pci.id_comision
    ...
    function lista_estatus_descuentos(){
        return $this->db->query(" SELECT * FROM opcs_x_cats where id_catalogo=23 and id_opcion in(18,19,20,21,22,23,24,25,26,29, 30,89)");
    }
    ...
    public function getUsuariosByComisionesAsistentes($idUsuarioSelect, $proyecto, $estatus)
    {
        ...
        $query =  $this->db->query("SELECT pci1.id_pago_i, pci1.id_comision, 
        (CASE WHEN com.ooam = 2 THEN CONCAT(lo.nombreLote,' <i>(',com.loteReubicado,')</i>') ELSE lo.nombreLote END) nombreLote, 
        re.nombreResidencial as proyecto, co.nombre as condominio,lo.totalNeto2 precio_lote, com.comision_total, com.porcentaje_decimal, 
        ...
        (CASE WHEN com.ooam = 1 THEN  CONCAT(oxcest.nombre,' (EEC)') ELSE oxcest.nombre END) estatus_actual, 
        oxcest.id_opcion id_estatus_actual, pci1.descuento_aplicado, 
        (CASE WHEN cl.lugar_prospeccion IS NULL THEN 0 ELSE cl.lugar_prospeccion END) lugar_prospeccion, lo.referencia, pac.bonificacion, u.estatus as activo, 
        (CASE WHEN pe.id_penalizacion IS NOT NULL THEN 1 ELSE 0 END) penalizacion, oxcest.color,
        (CASE WHEN cl.proceso = 0 THEN '' ELSE oxc0.nombre END) procesoCl,
        (CASE WHEN cl.proceso = 0 THEN '' ELSE 'label lbl-violetBoots' END) colorProcesoCl, cl.proceso, 
        ISNULL(cl.id_cliente_reubicacion_2, 0) id_cliente_reubicacion_2
        FROM pago_comision_ind pci1 
        LEFT JOIN (SELECT SUM(abono_neodata) abono_pagado, id_comision 
        FROM pago_comision_ind WHERE (estatus in (11,3) OR descuento_aplicado = 1) 
        GROUP BY id_comision) pci2 ON pci1.id_comision = pci2.id_comision
        ...
        GROUP BY pci1.id_comision,com.ooam,com.loteReubicado, lo.nombreLote, re.nombreResidencial, co.nombre, lo.totalNeto2, com.comision_total, com.porcentaje_decimal,
        pci1.abono_neodata,
        pci1.pago_neodata, pci2.abono_pagado, pci1.estatus, pci1.fecha_abono, pci1.id_usuario, pci1.id_pago_i, u.nombre, u.apellido_paterno, u.apellido_materno, 
        oprol.nombre, oxcest.nombre, oxcest.id_opcion, pci1.descuento_aplicado, cl.lugar_prospeccion, lo.referencia, com.estatus, 
        pac.bonificacion, u.estatus,pe.id_penalizacion, 
        oxcest.color, cl.estructura, oprol2.nombre, cl.proceso, oxc0.nombre, cl.id_cliente_reubicacion_2 ORDER BY lo.nombreLote");
...
function insert_penalizacion_individual($id_comision, $id_usuario, $rol, $abono_nuevo, $pago, $idCliente){
   ...
   if (! $respuesta ) {
    ...
    $mesActual = date('m');
    $filtro = ($tipoUsuario == 2 || $tipoUsuario == 4) ?  ( $diaActual <= 15 ? "AND Day(fechaInicio) <= 17" : "AND Day(fechaInicio) >= 17" ) : "";
    $filtro2 = $this->session->userdata('id_sede') == 8 ? ",fechaTijuana AS fechaFin" : ",fechaFinGeneral AS fechaFin";
      return $consultaFechasCorte = $this->db->query("SELECT mes,fechaInicio,tipoCorte $filtro2 FROM fechasCorte WHERE estatus = 1 AND 
      tipoCorte IN($tipoUsuario) AND YEAR(GETDATE()) = YEAR(fechaInicio) AND mes = $mesActual $filtro ORDER BY tipoCorte ASC")->result_array();
    }
    ...
    public function getReporteDesc($sede, $empresa, $puesto, $usuario, $beginDate, $endDate){
        ...
        return $this->db->query("SELECT pa.evidenciaDocs as evidencia,mrp.evidencia as relacion_evidencia,pa.id_prestamo,pci.id_pago_i,pci.id_pago_i,lo.idLote,lo.nombreLote,u.id_usuario,CONCAT(u.nombre,' ',u.apellido_paterno,' ',u.apellido_materno) nombre, pa.monto,pa.num_pagos,pa.n_p,pa.fecha_creacion,pa.fecha_modificacion,
