		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form method="post" id="form_descuentos">
                    ...
                    <select id="idloteorigen" name="idloteorigen" class="form-control directorSelect ng-invalid ng-invalid-required" required data-live-search="true"></select>
                    </div>
                    ...
                    <div class="form-group row">
                       ...
                       <div class="col-md-6">
                       <div class="form-group" id="montodisponible">
															...
															<div class="card">
																<div class="card-header">
                                                                    ...
                                                                    <div class="col-md-12 text-right">
                                                                        <button ype="button" class="btn btn-primary " data-toggle="modal" data-target="#miModal">AGREGAR DESCUENTO</button>
                                                                        </div>

                                                                        <label style="color: #0a548b;">&nbsp;Descuentos en revisión<b>:</b> $<input style="border-bottom: none; border-top: none; border-right: none;  border-left: none; background: white; color: #0a548b; font-weight: bold;" disabled="disabled" readonly="readonly" type="text"  name="totalp" id="totalp"></label>
				...
				},
				{
					...
					"data": function( d ){
                        ...
                        // if(d.estatus==0){
                            ...
                            '<button class="btn btn-default btn-round btn-fab btn-danger btn-fab-mini btn-delete" value="'+d.id_pago_i+'"  style="margin-right: 3px;"><i class="material-icons" data-toggle="tooltip" data-placement="right" title="ELIMINAR">delete_sweep</i></button>';
...
$("#form_aplicar").submit( function(e) {
    ...
    $("#usuarioid").change(function() {
        ...
        $.post('getLotesOrigen/'+user, function(data) {
                        ...
                        for( var i = 0; i<len; i++)
                        {
                            ...
                            var comision = data[i]['id_comision'];
 
                             
                            $("#idloteorigen").append($('<option>').val(comision).text(name));
                        }
                        if(len<=0)
                        {
    ...
    $("#idloteorigen").change(function() {
        ...
        $.post('getInformacionData/'+comision, function(data) {

            var disponible = (data[0]['comision_total']-data[0]['abono_pagado']);
            var idecomision = data[0]['id_comision'];
...
function verificar(){
    let disponible = parseFloat($('#valor_comision').val());
    let monto = parseFloat($('#monto').val());
