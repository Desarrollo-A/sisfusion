...
class Comisiones extends CI_Controller
{
  ...
  public function acepto_comisiones_user(){
    ...
    if(in_array($consulta_comisiones->result_array()[0]['forma_pago'],$formaPagoInvalida)){//EL COMISIONISTA SI TIENE UNA FORMA DE PAGO VALIDA Y CONTINUA CON EL PROCESO DE ENVIO DE COMISIONES
      ...
      $consultaFechasCorte = $this->db->query("SELECT * FROM fechasCorte WHERE estatus = 1 AND corteOoam = ".$consultaTipoUsuario[0]['tipo']." AND mes = $mesActual")->result_array();
  ...
  public function getDesarrolloSelect($a = ''){
    $validar_sede = $this->session->userdata('id_sede');
    $mesActual = $this->db->query("SELECT MONTH(GETDATE()) AS mesActual")->row()->mesActual;
    $consultaFechasCorte = $this->db->query("SELECT * FROM fechasCorte WHERE  estatus = 1 AND mes=$mesActual")->result_array();
    $obtenerFechaSql = $this->db->query("select FORMAT(CAST(FORMAT(SYSDATETIME(), N'yyyy-MM-dd HH:mm:ss') AS datetime2), N'yyyy-MM-dd HH:mm:ss') as sysdatetime")->row()->sysdatetime;   
    $fecha_actual = strtotime($obtenerFechaSql);
    $fechaInicio = strtotime($consultaFechasCorte[0]['fechaInicio']);
    $fechaFin = $validar_sede == 8 ? strtotime($consultaFechasCorte[0]['fechaTijuana']) : strtotime($consultaFechasCorte[0]['fechaFinGeneral']) ;
  ...
  public function cargaxml2($id_user = ''){
    ...
    if( isset( $_FILES ) && !empty($_FILES) ){
      ...
      $config['allowed_types'] = 'xml'; //CARGAMOS LA LIBRERIA CON LAS CONFIGURACIONES PREVIAS -----$this->upload->display_errors()
  ...
  public function guardar_solicitud2($usuario = '', $tipo = ''){
    $validar_sede =   $usuarioid =$this->session->userdata('id_sede');
    $mesActual = $this->db->query("SELECT MONTH(GETDATE()) AS mesActual")->row()->mesActual;
    $consultaFechasCorte = $this->db->query("SELECT * FROM fechasCorte WHERE estatus = 1 AND corteOoam = $tipo AND mes = $mesActual")->result_array();
    $obtenerFechaSql = $this->db->query("select FORMAT(CAST(FORMAT(SYSDATETIME(), N'yyyy-MM-dd HH:mm:ss') AS datetime2), N'yyyy-MM-dd HH:mm:ss') as sysdatetime")->row()->sysdatetime;   
    $fecha_actual = strtotime($obtenerFechaSql);
    $fechaInicio = strtotime($consultaFechasCorte[0]['fechaInicio']);
    $fechaFin = $validar_sede == 8 ? strtotime($consultaFechasCorte[0]['fechaTijuana']) : strtotime($consultaFechasCorte[0]['fechaFinGeneral']) ;
  ...
  public function saveDescuento($valor) {
    ...
    if(floatval($valor) == 1){
      ...
      $descuent0 = str_replace(",",'',$descuento);
      $descuento = str_replace("$",'',$descuent0);
    } else if(floatval($valor) == 3){/**DESCUENTOS UNIVERSIDAD*/
  ...
  public function getDescuentosCapitalpagos(){
    echo json_encode( $this->Comisiones_model->getDescuentosCapitalpagos( $this->input->post("id_usuario") ) );
  }
    ...
    if ($type_transaction == 1) { // ADD MKTD
      ...
      if (COUNT($commission_data) > 0) { // MJ: SE ENCONTRÓ REGISTRO EN COMISIONES
        ...
        if (COUNT($cce_data) <= 0) { // MJ: SI HAY REGISTRO DE COMISIONES, ENTONCES NO INSERTAMOS LA EMPRESA SINO SÍ 
          ...
          if(count($cce_data) == 1){
            if($cce_data[0]['id_usuario'] == 4824){  //VERIFICAR SI HAY PAGOS DE EMPREA, SI HAY PAGOS QUE NO SEAN NUEVOS, SE TOPA LA EMPRESA SI NO SE CAMBIOA DE USUARIO
              ...
              if (COUNT($pci_data) > 0) { // MJ: SE ENCONTRARON REGISTROS EN pago_comision_ind CON ESTATUS PAGADO
                ...
                $this->Comisiones_model->addRecord("comisiones", $insert_comisiones_data); // MJ: LLEVA 2 PARÁMETROS $table, $data                
              } else{// MJ: SE OBTIENEN TODOS LOS PAGOS DISTINTOS A 11 Y 8
                $pci_others_data = $this->Comisiones_model->getIndividualPaymentWPInformation($id_lote, 4824);
                if(count($pci_others_data) > 0){  //HAY PAGOS QUE SE PUEDEN EDITAR
      ...
      if (COUNT($commission_data) > 0) { // MJ: SE ENCONTRARON REGISTROS EN pago_comision_ind CON ESTATUS PAGADO MO - SI SE ENCONTRO REGISTRO EN COMISIONES SE TOPA LA COMISIÓN 
        ...
        $this->Comisiones_model->updateRecord("clientes", $update_client_data, "id_cliente", $lote_data[0]['idCliente']); // MJ: LLEVA 4 PARÁMETROS $table, $data, $key, $value
        $this->Comisiones_model->updateRecord("prospectos", $update_client_data, "id_prospecto", $lote_data[0]['id_prospecto']); // MJ: LLEVA 4 PARÁMETROS $table, $data, $key, $value
      }// MJ: INSERT PARA AGREGAR LA EMPRESA A COMISIONES  MJ: SE CAMBIA LUGAR DE PROSPECCIÓN DEL CLIENTE DE 6 A 11
  ...
  /**-------------------------MKTD COMPARTIDAS------------------------ */
  public function MKTD_compartida(){ /**SE VERIFICA SI MKTD TIENE PAGOS DE ESTE LOTE EN OTROS ESTATUS QUE NO SEA NUEVO */
    ...
    if ($diaActivacion >= 1 && $diaActivacion <= 5) {
      if ($mesActivacion === $mesActual) {
        if ($anioActivacion === $anioActual) {
          $result = $this->Comisiones_model->updatePagoReactivadoMismoDiaMes($idDescuento, date('Y-m-d H:i:s', $fechaActivacion));
        } else {
          $result = $this->Comisiones_model->updatePagoReactivadoFechaDiferente($idDescuento, date('Y-m-d H:i:s', $fechaActivacion));
        }
      } else {
        $result = $this->Comisiones_model->updatePagoReactivadoFechaDiferente($idDescuento, date('Y-m-d H:i:s', $fechaActivacion));
      }
    } else if ($mesActivacion === $mesActual) {
      $result = $this->Comisiones_model->updatePagoReactivadoMismoMes($idDescuento, date('Y-m-d H:i:s', $fechaActivacion));
    } else {
      $result = $this->Comisiones_model->updatePagoReactivadoFechaDiferente($idDescuento, date('Y-m-d H:i:s', $fechaActivacion));
    }
    echo json_encode($result);
  ...
  public function changeLoteToPenalizacion(){
    $response = $this->Comisiones_model->insertHistorialLog($_POST['id_lote'], $this->session->userdata('id_usuario'), 1, 'SE ACEPTÓ PENALIZACIÓN', 'penalizaciones', 'NULL');
    if ($response) {
  ...
  public function changeLoteToPenalizacionCuatro(){
    $response = $this->Comisiones_model->insertHistorialLog($_POST['id_lote'], $this->session->userdata('id_usuario'), 1, 'SE ACEPTÓ PENALIZACIÓN + 160 DÍAS', 'penalizaciones', 'NULL');
    if ($response) {
  ...
  public function ultimoRegistro (){
    $idLote   = $this->input->post('idLote');
    $this->comisiones_model->ultimoRegistro($idLote);
  }
