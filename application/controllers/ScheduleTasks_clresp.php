<?php class ScheduleTasks_cl extends CI_Controller {
	...
	public function validaVencimiento()
	{
		...
		for($i=0; $i<count($data);$i++)
		{
			$id_prospecto = $data[$i]['id_prospecto'];
				...
				if($this->scheduleTasks_model->disableProspecto($id_prospecto, $dataUpdate))
				{
					$response = array(
						'success'	=>	'Se ha cambiado el estatus correctamente',
				...
				{
					$response = array(
						'error'	=>	'Ocurrio un error al cambiar de estatus el prospecto'
					);
					if($response != null) {
						echo json_encode($response);
					} else {
			...
			/*$diasTrans = $this->getDiffDays($data[$i]['fecha_vencimiento']);
			if($diasTrans>0)
			{
			}
			else{
	...
	public function getDiffDays($fecha_2)
	{
		...
		//$fecha2 = new dateTime('2020-07-16');
...
// https://maderascrm.gphsis.com/index.php/ScheduleTasks_cl/select_gph_maderas_67
  
public function select_gph_maderas_67(){ //HACER INSERT DE LOS LOTES EN 0 Y PASARLOS A 1

  $QUERY_DATA_GPHSIS = $this->gphsis->query("SELECT * FROM [GPHSIS].[dbo].[Comisiones] WHERE Saldorestante IS NOT NULL AND Montoultimoabono IS NOT NULL AND Marca IN (17) AND USR IS NOT NULL AND USR NOT IN (0) AND Referencia NOT IN (0,  1140094,  1150622,  5001029,  8020331)");
    ...
    // if($QUERY_DATA_GPHSIS->num_rows() > 0 and $QUERY_DATA_GPHSIS->num_rows() < 2){
      ...
      foreach ($QUERY_DATA_GPHSIS->result() as $row2) {
        ...
        if($REVISAR_QUERY->num_rows()<1){
          ...
          //     $dispersar = $row2->Montoultimoabono;
          //     $pauta_neodata = 0;
          //   }
        }
          ...
          echo '<br>'."NO HAY DATOS CON ESTA REFERENCIA";
                ...
                if($busquedaAsesor->num_rows() > 0){
                  ...
                  foreach($busquedaAsesor->result() as $as ){
                    ...
                    $this->db->query("INSERT INTO [pago_comision_ind] (id_comision, id_usuario, abono_neodata, fecha_abono, fecha_pago_intmex, estatus, pago_neodata) VALUES (".$as->id_comision.", ".$as->id_usuario.", ".$insert_new.",  GETDATE(),  GETDATE(), 58, ".$dispersar.")");
                  }
                }
                ...
                if($busquedaCoordinador->num_rows() > 0){
                  // for($c = 0; $c<$busquedaCoordinador->num_rows(); $c++){
                    ...
                    $this->db->query("INSERT INTO [pago_comision_ind] (id_comision, id_usuario, abono_neodata, fecha_abono, fecha_pago_intmex, estatus, pago_neodata) VALUES (".$co->id_comision.", ".$co->id_usuario.", ".$insert_new.",  GETDATE(),  GETDATE(), 58, ".$dispersar.")");
                  }
                }
                ...
                if($busquedaGerente->num_rows() > 0){
                  ...
                  $this->db->query("INSERT INTO [pago_comision_ind] (id_comision, id_usuario, abono_neodata, fecha_abono, fecha_pago_intmex, estatus, pago_neodata) VALUES (".$ge->id_comision.", ".$ge->id_usuario.", ".$insert_new.",  GETDATE(),  GETDATE(), 58, ".$dispersar.")");
                }
              }
                ...
                if($busquedaSubdirector->num_rows() > 0){
                  ...
                  $this->db->query("INSERT INTO [pago_comision_ind] (id_comision, id_usuario, abono_neodata, fecha_abono, fecha_pago_intmex, estatus, pago_neodata) VALUES (".$su->id_comision.", ".$su->id_usuario.", ".$insert_new.",  GETDATE(),  GETDATE(), 58, ".$dispersar.")");
                }
              }
                ...
                if($busquedaDirector->num_rows() > 0){
                  ...
                  $this->db->query("INSERT INTO [pago_comision_ind] (id_comision, id_usuario, abono_neodata, fecha_abono, fecha_pago_intmex, estatus, pago_neodata) VALUES (".$di->id_comision.", ".$di->id_usuario.", ".$insert_new.",  GETDATE(),  GETDATE(), 58, ".$dispersar.")");
                }
              }
                ...
                if($busquedaMktd->num_rows() > 0){
                  ...
                  $this->db->query("INSERT INTO [pago_comision_ind] (id_comision, id_usuario, abono_neodata, fecha_abono, fecha_pago_intmex, estatus, pago_neodata) VALUES (".$mk->id_comision.", ".$mk->id_usuario.", ".$insert_new.",  GETDATE(),  GETDATE(), 58, ".$dispersar.")");
                }
              }
              ...
              $this->gphsis->query("UPDATE [GPHSIS].[dbo].[Comisiones] SET Marca = ".$pauta_neodata." WHERE Referencia = ".$REF_001." AND Saldorestante IS NOT NULL AND Montoultimoabono IS NOT NULL AND Marca IN (17)");
