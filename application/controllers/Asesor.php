...
class Asesor extends CI_Controller {
    public function __construct() {
        ...
        $this->jwt_actions->authorize('4001', $_SERVER['HTTP_HOST']);
...
function getLotesInventarioGralc($residencial, $condominio){
    ...
    public function deposito_seriedad($id_cliente, $onlyView){      
        ...
        public function deposito_seriedad_second($id_cliente, $onlyView){
        $datos["cliente"]= $this->Asesor_model->selectDS($id_cliente);
        $datos["cliente"][0]->tipo_nc = $datos["cliente"][0]->tipo_nc==''?3:(int)$datos["cliente"][0]->tipo_nc;
        $datos["cliente"][0]->printPagare = $datos["cliente"][0]->printPagare==''?3:(int)$datos["cliente"][0]->printPagare;
    ...
    public function imprimir_ds($id_cliente){
        ...
        $costoM2 = ($informacion_cliente->row()->desarrollo == 17) ? $informacion_cliente->row()->costoM2_casas : $informacion_cliente->row()->costoM2;
            ...
            $ladaTel1 = ($informacion_cliente->row()->ladaTel1=='' || $informacion_cliente->row()->ladaTel1==NULL) ? '' : '(+'.$informacion_cliente->row()->ladaTel1.')';
            $ladaTel2 = ($informacion_cliente->row()->ladaTel2=='' || $informacion_cliente->row()->ladaTel2==NULL) ? '' : '(+'.$informacion_cliente->row()->ladaTel2.')';
...
$html = '<!DOCTYPE html>
            ...
            if ($informacion_cliente->row()->rfc != '' && $informacion_cliente->row()->rfc != null){
                ...
                <tr>
                    ...
                    if( $informacion_cliente->row()->cp_fac != 0 ){
                        $html .= '<td width="30%"><b>CODIGO POSTAL:</b> ' . $informacion_cliente->row()->cp_fac . '</td><td width="29%" colspan="2"></td>';
                    }
            ...
            <td width="33.3%" colspan="2" style="border-bottom: 1px solid #CCCCCC"><label>TELÉFONO CASA:
                </label><br><br><b style="padding-left: 250px">&nbsp;'.$ladaTel1.' '.$informacion_cliente->row()->telefono1.'</b><br>
            </td>
            <td width="33.3%" colspan="2" style="border-bottom: 1px solid #CCCCCC"><label>CELULAR (<b><span style="color: red;">*</span></b>) </label><br><br><b>&nbsp;'.$ladaTel2.' '.$informacion_cliente->row()->telefono2.'</b><br>
            </td>
    ...
    public function editar_ds(){
        ...
        if($numOfCoprops > 0) {
            for ($i = 0; $i < $numOfCoprops; $i++) {
                ...
                {
                    $array18[$i]  = $this->input->post("tipo_vivienda_cop" . $i . "[]")[0];
                }
            }
        }
        ...
        for ($c = 0; $c < count($regFiscal2); $c++){
            ...
            if(!$validacionM2) {//si es diferente a true
                $this->session->set_userdata('success_coprop', 400);
            ...
            $aportMensualOfer = str_replace(',','',$this->input->post('aportMensualOfer'));;
        ...
        /**********/
                if(count($array17) > 0)
                {
                    for ($i=0; $i < sizeof($array17); $i++) {
                        $updCoprop = $this->db->query("UPDATE copropietarios SET correo = '".$array1[$i]."', ladaTel = '" . $ladatelCop1[$i] . "', ladaCel = '" . $ladaCelCop2[$i] . "', telefono = '".$array2[$i]."', telefono_2 = '".$array3[$i]."', 
                            fecha_nacimiento = '".$array4[$i]."', nacionalidad = '".$array5[$i]."', originario_de = '".$array6[$i]."', domicilio_particular = '".$array7[$i]."', estado_civil = '".$array8[$i]."', conyuge = '".$array9[$i]."', regimen_matrimonial = '".$array10[$i]."', ocupacion = '".$array11[$i]."', posicion = '".$array12[$i]."', empresa = '".$array13[$i]."', antiguedad = '".$array14[$i]."', edadFirma = '".$array15[$i]."', direccion = '".$array16[$i]."',
        ...
        if(count($checkIfRefExist) >0)
        {
                   ...
                   if($updateDs)
                   {
                    ...
                    if($arreglo_cliente['tipo_comprobanteD'] == 1){
                     ...
                     {
                        for ($i=0; $i < sizeof($array17); $i++) {
                           $updCoprop = $this->db->query("UPDATE copropietarios SET correo = '".$array1[$i]."', telefono = '".$array2[$i]."', ladaTel='".$ladaTelCop1[$i]."',telefono_2 = '".$array3[$i]."', ladaCel='".$ladaCelCop2[$i]."',fecha_nacimiento = '".$array4[$i]."', nacionalidad = '".$array5[$i]."', originario_de = '".$array6[$i]."', domicilio_particular = '".$array7[$i]."', estado_civil = '".$array8[$i]."', conyuge = '".$array9[$i]."', regimen_matrimonial = '".$array10[$i]."', ocupacion = '".$array11[$i]."', posicion = '".$array12[$i]."', empresa = '".$array13[$i]."', antiguedad = '".$array14[$i]."', edadFirma = '".$array15[$i]."', direccion = '".$array16[$i]."',
                                rfc='".$array19[$i]."',  tipo_vivienda=".$array18[$i]."
                            WHERE id_copropietario = ".$array17[$i]);
...
public function envioRevisionAsesor2aJuridico7() {
  ...
  public function AdminCarpetas()
    ...
    function setControversias(){
        ...
        if ($data != null){
            ...
            if($data2 == null){
                $data_insert =  array(
                    'id_lote' => $idLote,
                    'tipo' => $this->input->post("controversy_type"),
                    'creado_por' => $this->session->userdata('id_usuario'),
    ...
    /*funciones de la facturacion en el ds*/
    function getPaises(){
        $data = $this->Asesor_model->getPaises();
        if($data != null) {
            echo json_encode($data);
        } else {
            echo json_encode(array());
    ...
    function getEstados(){
        $data = $this->Asesor_model->getEstados();
        if($data != null) {
            echo json_encode($data);
        } else {
            echo json_encode(array());
    ...
    function getCP($Estado){
        $data = $this->Asesor_model->getCP($Estado);
        if($data != null) {
            echo json_encode($data);
        } else {
            echo json_encode(array());
    ...
    function getColonia($CP){
        $data = $this->Asesor_model->getColonia($CP);
        if($data != null) {
            echo json_encode($data);
        } else {
            echo json_encode(array());
    ...
    function getTipoPersonalidadJuridica(){
        $data = $this->Asesor_model->getTipoPersonalidadJuridica();
        if($data != null) {
            echo json_encode($data);
        } else {
            echo json_encode(array());
    ...
    function getTMunicipioByIdMunicipio($idMunicipio){
        $data = $this->Asesor_model->getTMunicipioByIdMunicipio($idMunicipio);
        if($data != null) {
            echo json_encode($data);
        } else {
            echo json_encode(array());
    ...
    public function validarDocumentosEstatus2($idLote, $tipo_comprobante, $id_cliente): bool
    {
        ...
        if (in_array($this->session->userdata('id_rol'), [17, 70])) { // ES CONTRALORÍA
            ...
            /*if (in_array($dataClient[0]['proceso'], [2, 3, 4])) {
                if ($dataClient[0]['personalidad_juridica'] == 1) { // PARA PM TAMBIÉN PEDIMOS LA CARTA PODER
                    ...
                    $documentsNumber += in_array($dataClient[0]['proceso'], [2, 4]) ? 2 : 0;
                    $documentosExtra_label = in_array($dataClient[0]['proceso'], [2, 4]) ? ", CARTA PODER, RESCISIÓN DE CONTRATO FIRMADA, CONTRATO 1 CANCELADO, CONTRATO 2 CANCELADO" : "";
                }
                else { // SI ES PF SÓLO PEDIMOS LA CARTA
                    $documentosExtra = $dataClient[0]['proceso'] == 3 ? "" : ", 35";
                    $documentsNumber += $dataClient[0]['proceso'] == 3 ? 0 : 1;
                    $documentosExtra_label = $dataClient[0]['proceso'] == 3 ? "" : ", RESCISIÓN DE CONTRATO FIRMADA, CONTRATO 1 CANCELADO, CONTRATO 2 CANCELADO";
                }
            }*/

            #prueba
            #NO VALIDO $validacionRes =  $this->Asesor_model->residencialParaValidacion($idLote);//valida que el lote este en algun proyecto de león para pedir la corrida al inicio
            #NOVALIDO $validacionAceptados = array(3, 13, 22, 31);

            //estas lineas se deben descomentar cuando entre en operación el módulo
            ...
            //esto también se debe de descomentar
            /*if(in_array($validacionRes, $validacionAceptados)){// && $this->session->userdata('id_usuario') == 1297
            ...
            #prueba cf
            $error_message = "Asegúrate de incluir los documentos: IDENTIFICACIÓN OFICIAL$comprobante_domicilio_label $labelCorridaFinanciera $documentosExtra_label, RECIBOS DE APARTADO Y ENGANCHE Y DEPÓSITO DE SERIEDAD antes de llevar a cabo el avance.";
        ...
        //ESTE CAMBIO NO DEBERIA DE ESTAR EN PROD, LO COMENTO PARA QUE POSTERIORMENTE SE VEA QUE PAJO
