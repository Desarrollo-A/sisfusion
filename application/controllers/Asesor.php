            ...
            if (in_array($dataClient[0]['proceso'], [2, 3, 4])) {
                if ($dataClient[0]['personalidad_juridica'] == 1) { // PARA PM TAMBIÉN PEDIMOS LA CARTA PODER
                    $documentosExtra = in_array($dataClient[0]['proceso'], [2, 4])
                        ? ", 32, 34, 35, 41, 42, 43"
                        : ", 32";
                    $documentsNumber += in_array($dataClient[0]['proceso'], [2, 4]) ? 6 : 1;
                    $documentosExtra_label = in_array($dataClient[0]['proceso'], [2, 4])
                        ? ", CARTA, CARTA PODER, RESCISIÓN DE CONTRATO FIRMADA, CONTRATO ELEGIDO FIRMA CLIENTE, CONTRATO 1 CANCELADO, CONTRATO 2 CANCELADO"
                        : "CARTA";
                }
                else { // SI ES PF SÓLO PEDIMOS LA CARTA
                    $documentosExtra = ", 32, 35, 41, 42, 43";
                    $documentsNumber += 5;
                    $documentosExtra_label = ", CARTA, RESCISIÓN DE CONTRATO FIRMADA, CONTRATO ELEGIDO FIRMA CLIENTE, CONTRATO 1 CANCELADO, CONTRATO 2 CANCELADO";
                }
            }
