...
class ScheduleTasks_com extends CI_Controller{
    ...
    public function CleanMonthly(){ //CRON JOB 1 DE CADA MES, ACTUALIZA DESCUENTOS UNIVERSIDAD, OPINIÓN DE CUMPLIMIENTO, CÓDIGO POSTAL, FACTURAS Y COMISIONES QUE NO TIENEN SALDO O FUERON DADAS DE BAJA
    ...
    //Editar el total comisión en banderas, esto sirve para recalcular los montos totales si se ajusta alguno de los comisionistas o la comisión en general
    ...
    //Editar el total abonado en banderas, esto sirve para recalcular los montos totales si se ajusta alguno de los comisionistas o la comisión en general
    ...
    //Editar el pendiente en banderas, esto sirve para recalcular los montos totales si se ajusta alguno de los comisionistas o la comisión en general
    ...
    public function LlenadoPlan(){ //CRON PARA ASGNAR EL PLAN DE COMISION
    ...
    // ScheduleTasks_com/activar_bandera_neo PERMITE BAJAR LOS SALDOS DE NEODATA DIARIAMENTE, SE EJECUTA POR PROYECTO SOLO SI CUMPLE LAS CONDICIONES DE getLotesPagados()
        ...
        for($j = 1; $j < $DAT+1; $j++){
            ...
            if(count($datos) > 0){
                ...
                for($i = 0; $i < COUNT($datos); $i++){
                    ...
                    if(!empty($data)){
                        if($data[$i]->Marca == 1){
                            if($data[$i]->Aplicado > ($datos[$i]['ultimo_pago']+100)){ // SI SE REGRESA A DISPERSIÓN
                                ...
                                $this->ComisionesNeo_model->UpdateBanderaPagoComisionNO($datos[$i]['id_lote']); // NO SE REGRESA A DISPERSIÓN
                            }
                        }else{
                            $this->ComisionesNeo_model->UpdateBanderaPagoComisionNO($datos[$i]['id_lote']); // NO SE REGRESA A DISPERSIÓN
                        }
