...
class ScheduleTasks_Prestamos extends CI_Controller
{
    ...
    public function CierrePrestamo($id_prestamo){
        ...
        JOIN pago_comision_ind pci ON pci.id_pago_i = rpp.id_pago_i AND pci.estatus in(18,19,20,21,22,23,24,25,26,29,30) AND pci.descuento_aplicado = 1
            ...
            if(count($PagosByUSer) > 0){
                for ($n=0;$n<count($PagosByUSer);$n++) {
                    $Suma = $Suma + $PagosByUSer[$n]['abono_neodata'];
                        ...
                        if($Suma >= $pagoMensual){//SI EL PAGO MENSUAL DEL PRESTAMO SE CUBRE CON LA SUMA ACTUAL DE LOS PAGOS, ESTE SE TOMARA PARA HACER EL DESCUENTO DEL PRESTAMO
                            ...
                            $this->db->query("INSERT INTO  historial_comisiones VALUES ($insert_id, 1, GETDATE(), 1, 'ESTE ES EL RESTANTE A UN DESCUENTO, SALDO DISPONIBLE PARA COBRO')");//Proceso para cerrar el prestamo, si es que ya se pago por completo
                            $CierrePrestamo = $this->CierrePrestamo($data[$m]['id_prestamo']);

                            if($CierrePrestamo[0]['pagado'] > ($data[$m]['monto'] - 0.50)){// EL PAGO SE APLICO CORRECTAMENTE Y SE CIERRA EL PRÉSTAMO, AQUI EVALUAR SI SE PASO LA CANTIDAD, NO APLICAR EL PRÉSTAMO
                            ...
                            } else{ //EL PAGO MENSUAL SE APLICO CORRECTAMENTE Y EL PRÉSTAMO SIGUE ACTIVO
                            ...
                            $CierrePrestamo = $this->CierrePrestamo($data[$m]['id_prestamo']);
                            if($n == count($PagosByUSer) -1 ){ //YA NO HAY MAS PAGOS
                            ...
                            } else{ //EL SUMA ACOMULADA NO CUBRE EL MONTO CORRESPONDIENTE AL MES, Y SE TOMA POR COMPLETO EL PAGO
                                $this->db->query("UPDATE pago_comision_ind SET estatus=$tipo_prestamo,descuento_aplicado=1,modificado_por=1 WHERE id_pago_i = ".$PagosByUSer[$n]['id_pago_i']." ");
                                $this->db->query("INSERT INTO relacion_pagos_prestamo(id_prestamo,id_pago_i,estatus,creado_por,fecha_creacion,modificado_por,fecha_modificacion,np) values(".$data[$m]['id_prestamo'].",".$PagosByUSer[$n]['id_pago_i'].",1,1,GETDATE(),1,GETDATE(),".$data[$m]['n_p'].")");
                                $this->db->query("INSERT INTO historial_comisiones values(".$PagosByUSer[$n]['id_pago_i'].",1,GETDATE(),1,'DESCUENTO POR ".$data[$m]['nombre'].", TOTAL A DESCONTAR $".number_format($data[$m]['monto'], 2, '.', ',')." A ".$data[$m]['num_pagos']." MES(ES), MOTIVO DESCUENTO: ".$data[$m]['comentario']."')");
                            }
    ...
    function SendMailPrestamos($data){
        ...
        $this->email
            ->initialize()
            ->from('Ciudad Maderas')
            ->to('tester.ti2@ciudadmaderas.com','programador.analista5@ciudadmaderas.com', 'programador.analista16@ciudadmaderas.com')
