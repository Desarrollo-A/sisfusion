...
class ScheduleTasks_Prestamos extends CI_Controller
{
    ...
    function descuentos_aut($param = 0){ // SI NO SE RECIBE EL PARAMETRO ES LA 1° EJECUCIÓN(CRONJOB). SI SE RECIBE ES POR QUE ES POR PARDE DE SISTEMAS Ó CONTRALORÍA
        ...
        /*CONSULTAMOS LOS PRÉSTAMOS ACTIVOS. SI SE RECIBE EL PARAMETRO DE ENTRADA AGREGAMOS MAS CONDICIÓNES A LA CONSULTA*/
        $data = $this->db->query("SELECT p.*,opc.nombre 
                                    FROM prestamos_aut p 
                                    INNER JOIN opcs_x_cats opc ON opc.id_opcion=p.tipo AND opc.id_catalogo=23 AND opc.id_opcion != 28
                                    WHERE p.estatus = 1 
                                    $sqlQuery
        ...
        for ($m=0; $m <count($data); $m++){
            ...
            $pagoMensual = $param != 0 ? $data[$m]['pendiente']  : $pagoMensual;
            ...
            $this->db->trans_begin();
                if(count($PagosByUSer) > 0)
                {
                            ...
                            if($Suma >= $pagoMensual)
                            {
                                ...
                                $CierrePrestamo = $this->CierrePrestamo($data[$m]['id_prestamo']);
                                    if($CierrePrestamo[0]['pagado'] > ($data[$m]['monto'] - 0.50)){
                                        $this->db->query("UPDATE prestamos_aut SET modificado_por=1,pendiente=0,fecha_modificacion=GETDATE(),estatus=3,mensualidadOk=1 WHERE id_prestamo=".$data[$m]['id_prestamo']."");
                                    }else{
                                        $this->db->query("UPDATE prestamos_aut SET n_p=n_p+1,modificado_por=1,pendiente=0,fecha_modificacion=GETDATE(),mensualidadOk=1 WHERE id_prestamo=".$data[$m]['id_prestamo']."");
                                    }
                                    break;
                            }else{
                                $CierrePrestamo = $this->CierrePrestamo($data[$m]['id_prestamo']);
                                if($n == count($PagosByUSer) -1 ){
                                    ...
                                    $this->db->query("INSERT INTO relacion_pagos_prestamo(id_prestamo,id_pago_i,estatus,creado_por,fecha_creacion,modificado_por,fecha_modificacion,np) values(".$data[$m]['id_prestamo'].",".$PagosByUSer[$n]['id_pago_i'].",1,1,GETDATE(),1,GETDATE(),".$data[$m]['n_p'].")");
                                    $this->db->query("INSERT INTO historial_comisiones values(".$PagosByUSer[$n]['id_pago_i'].",1,GETDATE(),1,'DESCUENTO POR ".$data[$m]['nombre'].", TOTAL A DESCONTAR $".number_format($data[$m]['monto'], 2, '.', ',')." A ".$data[$m]['num_pagos']." MES(ES), MOTIVO DESCUENTO: ".$data[$m]['comentario']."')");
                                    $this->db->query("UPDATE prestamos_aut SET n_p=n_p+1,modificado_por=1,fecha_modificacion=GETDATE(),pendiente=$pendiente,mensualidadOk=0 WHERE id_prestamo=".$data[$m]['id_prestamo']."");
                                    $arrayCorreo[$m] = array(
                    ...
                    if($CierrePrestamo[0]['pagado'] > ($data[$m]['monto'] - 0.50)){
                        ...
                        $this->db->query("UPDATE prestamos_aut SET modificado_por=1,fecha_modificacion=GETDATE(),pendiente=$pendiente,mensualidadOk=0 WHERE id_prestamo=".$data[$m]['id_prestamo']."");
                    }
                    $arrayCorreo = array(
                        "id_prestamo" => $data[$m]['id_prestamo'],
                        "id_usuario" => $data[$m]['id_usuario'],
    ...
    function SendMailPrestamos($data){
        ...
        $this->email
            ->initialize()
            ->from('Ciudad Maderas')
            ->to(['programador.analista21@ciudadmaderas.com', 'programador.analista16@ciudadmaderas.com'])
