    ...
    public function validateUserAccess() {
        ...
        if (!isset($data->usuario) || !isset($data->contrasena))
            ...
            if ($data->usuario == '' || $data->contrasena == '')
                ...
                $result = $this->Suma_model->getUserInformation($data->usuario, encriptar($data->contrasena));
                if ($result->id_rol != 7)
                    echo json_encode(array("status" => 401, "message" => "El valor ingresado para OWNER no corresponde a un ID de usuario con rol de asesor."), JSON_UNESCAPED_UNICODE);
                else {
                    if ($result->estatus != 1)
