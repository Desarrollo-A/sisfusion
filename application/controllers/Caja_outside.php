...
//VERIFICAMOS EL ACCESO AL ASESOR MEDIANTE SU ID Y CONTRASEÑA
    ...
    /*SERVICIO DE MARIACHOISE*/
    public function sumServ()
    {
        $data_json = json_decode((file_get_contents("php://input")));
        $request = array();
        foreach ($data_json as &$data) {
            if (!isset($data->id_cliente) || !isset($data->total_venta) || !isset($data->comisionistas) || !isset($data->nombre_cliente)
                || !isset($data->id_pago) || !isset($data->referencia))
                array_push($request, array("status" => 400, "message" => "Algún parámetro no tiene un valor especificado o no viene informado."));
                //echo json_encode(array("status" => 400, "message" => "Algún parámetro no tiene un valor especificado o no viene informado."), JSON_UNESCAPED_UNICODE);
            else {
                if ($data->id_cliente == "" || $data->total_venta == "" || $data->comisionistas == "" || $data->nombre_cliente == ""
                    || $data->id_pago == ""|| $data->referencia == "")
                    array_push($request, array("status" => 400, "message" => "Algún parámetro no tiene un valor especificado o no viene informado..."));
                    //echo json_encode(array("status" => 400, "message" => "Algún parámetro no tiene un valor especificado o no viene informado..."), JSON_UNESCAPED_UNICODE);
                else {
                    if(count($data->comisionistas)==0){
                        array_push($request, array("status" => 400, "message" => "Debe haber al menos un comisionista."));
                        //echo json_encode(array("status" => 400, "message" => "Debe haber al menos un comisionista."), JSON_UNESCAPED_UNICODE);
                    }else{
                        $updateData = array(
                    ...
                    //$result = $this->General_model->updateRecord("condominios", $updateData, "idCondominio", $data->idCondominio);/
                        $result = true;
                        if ($result == true){
                            $flagIns = 0;
                            foreach ($data->comisionistas as &$valor) {
                                //insert de cada comisionista
                                $array_comisionistas = array(
                                    'id_rol' => $valor->id_rol,
                                    'id_usuario' => $valor->id_usuario,
                                    'porcentaje_comision' => $valor->porcentaje_comision,
                                    'total' => $valor->total_comision,
                                );


                                //$flagInsert = $this->General_model->insertRecord();
                                $flagInsert = true;
                                if($flagInsert==true) {$flagIns++;}
                            }
                            if(count($data->comisionistas) == $flagIns)
                                array_push($request, array("status" => 200, "message" => "El registro se ha actualizado de manera exitosa."));
                                //echo json_encode(array("status" => 200, "message" => "El registro se ha actualizado de manera exitosa."), JSON_UNESCAPED_UNICODE);
                            else
                                array_push($request, array("status" => 400, "message" => "Oops, ocurrió un error al insertar un comisionista. Inténtalo más tarde."));
                                //echo json_encode(array("status" => 400, "message" => "Oops, ocurrió un error al insertar un comisionista. Inténtalo más tarde."), JSON_UNESCAPED_UNICODE);
                        }
                        else
                            array_push($request, array("status" => 400, "message" => "Oops, algo salió mal. Inténtalo más tarde."));
                            //echo json_encode(array("status" => 400, "message" => "Oops, algo salió mal. Inténtalo más tarde."), JSON_UNESCAPED_UNICODE);
                    }
