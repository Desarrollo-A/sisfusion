...
class ComisionesFacturas extends REST_Controller {
	...
	public function index_get($id = 0)
	{
		if(!empty($id)){
			...
			us.apellido_materno) AS nombreComisionista, us.rfc, oprol.nombre as puesto, CONCAT(cli.nombre, ' ', cli.apellido_paterno, ' ', cli.apellido_materno) AS cliente,
				...
				(CASE us.forma_pago WHEN 3 THEN (((100-sed.impuesto)/100)*pci.abono_neodata) ELSE pci.abono_neodata END) comisionTotal, 
				 com.porcentaje_decimal as porcentajeComision, null as entradaVenta, oxc3.nombre as esquema, 'completa' AS cobro, xm.stringInformacion as xmldata, pci.id_pago_i as idPago 	
					FROM pago_comision_ind pci
					 ...
					 INNER JOIN usuarios us ON us.id_usuario = com.id_usuario AND us.forma_pago in (2)
	 ...
	 ELSE us.id_sede END) and sed.estatus = 1
				...
				pci.abono_neodata, pci.abono_neodata, com.porcentaje_decimal, oxc3.nombre, xm.stringInformacion
