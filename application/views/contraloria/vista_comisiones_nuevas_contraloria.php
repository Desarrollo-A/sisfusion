...
<div class="wrapper ">
    ...
    <div class="modal fade modal-alertas" id="modal_comisiones" role="dialog" data-backdrop="static" data-keyboard="false">
                ...
                <div class="col xol-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <div class="card">
                        <div class="card-header card-header-icon" data-background-color="blue">
                            COMISIONES <b>NUEVAS</b>
                             <span style="font-size: 10px;"> (Recepción de nuevas solicitudes)</span>
                        </div>
...
<script>
    ...
    $('#tabla_nuevas_comisiones thead tr:eq(0) th').each( function (i) {
       ...
       if(i != 0 && i != 11){
        ...
        $( 'input', this ).on('keyup change', function () {
            if (tabla_comisiones.column(i).search() !== this.value ) {
                tabla_comisiones
                .column(i)
                .search(this.value)
                .draw();
    ...
    tabla_comisiones = $("#tabla_nuevas_comisiones").DataTable({
      // dom: 'Brtip',
      width: 'auto',
...
"order": [[ 1, 'asc' ]]
 ...
 $('#tabla_nuevas_comisiones tbody').on('click', 'td.details-control', function () {
    var tr = $(this).closest('tr');
    var row = tabla_comisiones.row( tr );
    ...
    $("#tabla_nuevas_comisiones tbody").on("click", ".agregar_estatus_comisiones", function(){

        nombreLote = $(this).val();
        IDLote =  $(this).attr("data-value");

        $("#modal_comisiones .modal-body").html("");

        $("#modal_comisiones .modal-body").append('<div class="row"><div class="col-lg-12"><label><h4>¿Está seguro de registrar el ingreso de <b>'+nombreLote+'</b>?</h4></label></div></div>');

        $("#modal_comisiones .modal-body").append('<div class="row"><div class="col-lg-2">&nbsp;</div><div class="col-lg-8"><center><button class="btn btn-social btn-fill btn-info boton_aceptar" style="background:green;margin-right: 10px;" value="'+IDLote+'">ACEPTAR</button><button class="btn btn-social btn-fill btn-danger" data-dismiss="modal" >CANCELAR</button></center></div></div>');
        $("#modal_comisiones").modal();
});
...
$(document).on("click", ".boton_aceptar", function(){
  ...
  $(window).resize(function(){
        tabla_comisiones.columns.adjust();
    });
