                ...
                <div class="col col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-content" id="liquidadoDesc" style="display: none">
                            ...
                            <div class="material-datatables">
                                <div class="table-responsive">
                                    <table  id="tabla_descuentos_liquidados" name="tabla_descuentos"
                                            class="table-striped table-hover" style="text-align:center;">
                                        <thead>
                                        <tr>
                                            <th>ID</th>
...
<script>
    ...
    $('#tabla_descuentos_liquidados thead tr:eq(0) th').each(function (i) {
        if (i != 12) {
            ...
            $('input', this).on('keyup change', function () {
                ...
                if (tabla_nuevas_liquidado.column(i).search() !== this.value) {
                    tabla_nuevas_liquidado
                        .column(i)
                        .search(this.value)
                        .draw();

                    var totalLIQ = 0;
                    var index = tabla_nuevas_liquidado.rows({selected: true, search: 'applied'}).indexes();
                    var data = tabla_nuevas_liquidado.rows(index).data();
    ...
    $('#tabla_descuentos_liquidados').on('xhr.dt', function (e, settings, json, xhr) {
        var total = 0;
        ...
        if (row.child.isShown()) {
            ...
            if (row.data().solicitudes == null || row.data().solicitudes == "null") {
                $.post(url + "Comisiones/getDescuentosCapitalpagos", {"id_usuario": row.data().id_usuario}).done(function (data) {
                    ...
                    row.child(construir_subtablas_liq(row.data().solicitudes)).show();
                    tr.addClass('shown');
    ...
    function construir_subtablas_liq(data) {
   ...
   /* $("#tabla_descuentos").ready(function () {
    ...
    /*********************************DESCUENTO UNIVERSADAD LIQUIDADOS************************************/




    /**AQUÍ EMPIEZA LO DE LA TABLA COMBINADO**/
    ...
    $('#tabla_descuentos_combinada thead tr:eq(0) th').each(function (i) {
        if (i!=13) {
            ...
            $('input', this).on('keyup change', function () {
                ...
                if (tabla_nuevas_combinada.column(i).search() !== this.value) {
                    tabla_nuevas_combinada
                        .column(i)
                        .search(this.value)
                        .draw();

                    var totalCombinado = 0;
                    var index = tabla_nuevas_combinada.rows({selected: true, search: 'applied'}).indexes();
                    var data = tabla_nuevas_combinada.rows(index).data();
    ...
    $('#tabla_descuentos_combinada').on('xhr.dt', function (e, settings, json, xhr) {
        var total = 0;
        $.each(json.data, function (i, v) {
            total += parseFloat(v.monto);
        });
    ...
    tabla_nuevas_combinada = $("#tabla_descuentos_combinada").DataTable({
        ...
        "columns": [
...

            {
                ...
                "data": function (d) {
                    if(d.queryType==2){
                        return '<p style="font-size: 1em"><b>---</b></p>';
                    }
    ...
    $("#tabla_descuentos_combinada tbody").on("click", ".consultar_logs_asimilados", function (e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        ...
        if (row.child.isShown()) {
            ...
            if (row.data().solicitudes == null || row.data().solicitudes == "null") {
                $.post(url + "Comisiones/getDescuentosCapitalpagos", {"id_usuario": row.data().id_usuario}).done(function (data) {
                    ...
                    tabla_nuevas_combinada.row(tr).data(row.data());

                    row = tabla_nuevas_combinada.row(tr);

                    row.child(construir_subtablas_combinada(row.data().solicitudes)).show();
                    tr.addClass('shown');
                    $(this).parent().find('.animacion').removeClass("fa-caret-right").addClass("fa-caret-down");
    ...
    $("#tabla_descuentos_combinada tbody").on("click", ".agregar_nuevo_descuento", function (e) {
        e.preventDefault();
        e.stopImmediatePropagation();
    ...
    $("#tabla_descuentos_combinada tbody").on("click", ".consultar_historial_pagos", function (e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        document.getElementById('nameUser').innerHTML = '';
        document.getElementById('montito').innerHTML = '';
    ...
    $("#tabla_descuentos_combinada tbody").on("click", ".topar_descuentos", function () {
        var tr = $(this).closest('tr');
        var row = tabla_nuevas_combinada.row(tr);
        id_usuario = $(this).val();
    ...
    function construir_subtablas_combinada(data) {
        var solicitudes = '<table class="table">';
        $.each(data, function (i, v) {
            //i es el indice y v son los valores de cada fila
