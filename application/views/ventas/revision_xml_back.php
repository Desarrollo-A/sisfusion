...
<div class="wrapper">
   ...
   if($this->session->userdata('id_rol')=="13" || $this->session->userdata('id_rol')=="17" || $this->session->userdata('id_rol')=="31" || $this->session->userdata('id_rol')=="32")//contraloria
     {
        /*-------------------------------------------------------*/
    $datos = array();
    $datos = $datos4;
    ...
    {
        if(($this->session->userdata('id_rol')=="2" || $this->session->userdata('id_rol')=="3" || $this->session->userdata('id_rol')=="3" || $this->session->userdata('id_rol')=="9") && $this->session->userdata('forma_pago')==2){
            $datos = array();
            $datos = $datos4;
            $datos = $datos2;
            $datos = $datos3;
    ...
    <div class="modal fade" id="seeInformationModalPDF" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-md modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer" id="pdffooter">
                <!-- <button type="button" class="btn btn-danger btn-simple" data-dismiss="modal" onclick="cleanCommentsfactura()"><b>Cerrar</b></button>
                <button type="button" class="btn btn-danger btn-simple" data-dismiss="modal" onclick="cleanCommentsfactura()"><b>Cerrar</b></button> -->
                </div>
                </form>
...
"defaultContent": '<i class="material-icons" style="color:#003D82;" title="Click para más detalles">play_circle_filled</i>'
        ...
        if(rol == 2 || rol == 3 || rol == 7 || rol==9){
            ...
            if(data.bandera != 3 ){
                     btnpdf = '<button value="'+data.uuid+'" data-userfactura="'+data.usuario+'" data-file="'+data.xmla+'" class="btn btn-round btn-fab btn-fab-mini subirPDF"  style="background: #3912C0;" title="Subir PDF">' +'<span class="material-icons">publish</span></button>';

                }else{
                    btnpdf = '<a class="btn btn-success btn-round btn-fab btn-fab-mini verPDF2" title= "Ver pdf" style="margin-top:5px;" data-usuario="'+namefile2[0]+'" ><i class="material-icons">library_books</i></a>';
                }
        }   else{

         
                        
                        BtnStats = '<button href="#" value="'+data.uuid+'" data-value="'+data.idResidencial+'" data-userfactura="'+data.usuario+'" data-code="'+data.cbbtton+'" ' +'class="btn btn-round btn-fab btn-fab-mini consultar_documentos"  style="background: #3982C0;" title="Detalle de factura">' +'<span class="material-icons">info</span></button>&nbsp;<a href="#" class="btn btn-info btn-round btn-fab btn-fab-mini verPDF" title= "Ver opinión de cumplimiento" style="margin-top:0px;" data-usuario="'+data.archivo_name+'" ><i class="material-icons">description</i></a>';
...
if(data.bandera != 3 ){
    ...
    btnpdf = '<a class="btn btn-success btn-round btn-fab btn-fab-mini verPDF2" title= "Ver pdf" style="margin-top:5px;" data-usuario="'+namefile[0]+'" ><i class="material-icons">library_books</i></a>';
}
                  


                        btnpausar = '<button value="'+data.uuid+'" data-id_user="'+data.id_usuario+'" data-userfactura="'+data.usuario+'" data-total="'+data.total+'" class="btn btn-round btn-fab btn-fab-mini regresar"  style="background: #7812C0;" title="Refacturar">' +'<span class="material-icons">autorenew</span></button>';
                        ...
                        BtnStats = '<button value="'+data.uuid+'" data-value="'+data.idResidencial+'" data-userfactura="'+data.usuario+'" data-code="'+data.cbbtton+'" ' +'class="btn btn-round btn-fab btn-fab-mini consultar_documentos"  style="background: #3982C0;" title="Detalles">' +'<span class="material-icons">info</span></button>&nbsp;';

                    }

                    
                   
                    return BtnStats+btnpdf+btnpausar;
...
<textarea id="motivo" name="motivo" class="form-control"></textarea>
  ...
  $("#seeInformationModalPDF .modal-body").append(`
                    ...
                    <button class="btn btn-warning" type="button" onclick="xml2(${id_user})" id="cargar_xml2"><i class="fa fa-upload">
