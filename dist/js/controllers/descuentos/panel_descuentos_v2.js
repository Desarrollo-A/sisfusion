...
$(document).ready(function() {    
    $.post(general_base_url+"Descuentos/getRolesIn",{ catalogo:1, roles:"1,2,3,7,9,38 "}, function (data) {
        var len = data.length;
...
$("#form_descuentosModal2").on('submit', function(e){
    $("#idloteorigen").prop("disabled", false);
    e.preventDefault();
    document.getElementById('btn_abonar').disabled=true;
    let formData = new FormData(document.getElementById("form_descuentosModal2"));
    $.ajax({
        url: 'saveDescuento/'+1,
        data: formData,
        method: 'POST',
...
$("#roles").change(function() {
    var parent = $(this).val();
    document.getElementById('montoContraloria').value = ''; 
    document.getElementById('idmontodisponible').value = ''; 
    document.getElementById('comentarioDescuentoM2').value = '';
    document.getElementById('sumaReal').innerHTML = '';
    $('#idloteorigen option').remove();
...
$("#idloteorigen").select2({dropdownParent:$('#miModal2')});
$("#idloteorigen2").select2({dropdownParent:$('#miModal3')}); 
$("#usuarioid").change(function() {
    ...
    document.getElementById("montoContraloria").focus();
...
$("#numeroP").change(function(){
    let monto = parseFloat($('#montoContraloria').val());
...
function verificar(){
    ...
    let monto = remplazarCaracter($('#montoContraloria').val(), ',', '');
