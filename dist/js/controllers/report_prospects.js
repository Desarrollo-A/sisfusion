...
sp = { //  SELECT PICKER
            ...
            },
            {
                data: function(d) {
                    if(d.tipo_controversia == 1){
                        return '<span class="label" style="background:#2ECC71;">Normal</span>';
                    }
                    else if(d.tipo_controversia == 2){
                        return '<span class="label" style="background:gold;">Para descuento</span>';
                    }
                    else if(d.tipo_controversia == 3){
                        return '<span class="label" style="background:#0b2c52;">Venta nueva</span>';
                    }
                    else{
                        return 'N/A';
                    }
                ...
                data: function(d) {
                    if( d.lugar_prospeccion != 6 ){
                        if(d.tipo_controversia == '' || d.tipo_controversia == null){
                            return '<div class="d-flex justify-center"><button class="btn-data btn-blueMaderas add_controversy" data-id-cliente="' + d.id_cliente + '" data-id-lote="' + d.idLote + '" data-nombre="' + d.nombreLote + '"  rel="tooltip" data-placement="left" title="Añadir controversia"><i class="fas fa-comment-medical"></i></button></div>';
                        }
                        else{
                            return '<div class="d-flex justify-center"><button class="btn-data btn-green add_evidence" data-id-cliente="' + d.id_cliente + '" data-id-lote="' + d.idLote + '" data-nombre="' + d.nombreLote + '"  rel="tooltip" data-placement="left" title="Subir evidencia"><i class="fas fa-cloud-upload-alt"></i></button></div>';
                        }
                    }
                    else{
                        return '';
    ...
    function buildTableDetail(data) {
        ...
        solicitudes += '<td>' + '<b>' + 'LUGAR PROSPECCION ' + '</b></td>';
        solicitudes += '<td>' + '<b>' + 'FECHA ALTA ' + '</b></td>';
    ...
    function getAsesores() {
        ...
        $("#asesores").append($('<option disabled>').val("0").text("Seleccione una opción"));
    ...
    function getSedes(){
        ...
        $("#sedess").append($('<option disabled>').val("0").text("Seleccione una opción"));
        $.post('getSedes', function(data) {
            var len = data.length;
            for (var i = 0; i < len; i++) {
                var id_sede = data[i]['id_sede'];
                var nombre = data[i]['nombre'];
                $("#sedess").append($('<option>').val(id_sede).text(nombre));
            }
