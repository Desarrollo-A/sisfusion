var tr;
var tabla_xml_seguros ;
let titulos_seguros = [];

$(document).ready(function() {
    $("#tabla_xml_seguros").prop("hidden", true);
    $.post(general_base_url+"Contratacion/lista_proyecto", function (data) {
        var len = data.length;
        for (var i = 0; i < len; i++) {
            var id = data[i]['idResidencial'];
            var name = data[i]['descripcion'];
            $("#proyectoXmlSeguros").append($('<option>').val(id).text(name.toUpperCase()));
        }
        $("#proyectoXmlSeguros").selectpicker('refresh');
    }, 'json');
});

$('#proyectoXmlSeguros').change(function(){
    proyectoSeguros = $('#proyectoXmlSeguros').val();
    getDataXMLSeguros(proyectoSeguros);
});

$('#tabla_xml_seguros thead tr:eq(0) th').each(function (i) {
    if(i != 0){
        var titleSeguros = $(this).text();
        titulos_seguros.push(titleSeguros);
        $(this).html('<input type="text" class="textoshead" data-toggle="tooltip" data-placement="top" title="' + titleSeguros + '" placeholder="' + titleSeguros + '"/>');
        $( 'input', this ).on('keyup change', function () {
            if ($('#tabla_xml_seguros').DataTable().column(i).search() !== this.value ) {
                $('#tabla_xml_seguros').DataTable().column(i).search(this.value).draw();
            }
        });
    }
});

function changeNameSeguros(e){
    const fileName = e.files[0].name;
    let relatedTarget = $( e ).closest( '.file-gph' ).find( '.file-name' );
    relatedTarget[0].value = fileName;
}

function getDataXMLSeguros(proyecto){
    $('#tabla_xml_seguros').on('xhr.dt', function(e, settings, json, xhr) {
        var total = 0;
        $.each(json.data, function(i, v) {
            total += parseFloat(v.total);
        });
        var to = formatMoney(numberTwoDecimal(total));
        document.getElementById("disponibleXmlSeguro").textContent = to;
    });

    $("#tabla_xml_seguros").prop("hidden", false);
    tabla_xml_seguros = $("#tabla_xml_seguros").DataTable({
        ...
        },
        {
            ...
            data: function( data ){
                ...
                if(data.estatus_opinion == 1 || data.estatus_opinion == 2){
                    BtnStats = '<button href="#" value="'+data.uuid+'" data-value="'+data.idResidencial+'" data-userfactura="'+data.usuario+'" data-code="'+data.cbbtton+'" ' +'class="btn-data btn-blueMaderas consultar_documentos_seguros" data-toggle="tooltip" data-placement="top" title="Detalle de factura">' +'<i class="fas fa-info"></i></button><a href="#" class="btn-data btn-warning verPDFSeguros" data-toggle="tooltip" data-placement="top" title= "Ver opinión de cumplimiento" data-usuario="'+data.archivo_name+'" ><i class="material-icons">description</i></a>';
                    ...
                    BtnStats = '<button value="'+data.uuid+'" data-value="'+data.idResidencial+'" data-userfactura="'+data.usuario+'" data-code="'+data.cbbtton+'" ' +'class="btn-data btn-blueMaderas consultar_documentos_seguros" data-toggle="tooltip" data-placement="top" title="Detalles">' +'<i class="fas fa-info"></i></button>';
                }
        ...
        ajax: {
            "url": general_base_url + "SegurosComision/getDatosNuevasXContraloria/",
            "type": "POST",
            cache: false,
            data:{
                proyecto:proyecto,
    ...
    $('#tabla_xml_seguros').on('draw.dt', function() {
        $('[data-toggle="tooltip"]').tooltip({
            trigger: "hover"
        });
    });

    $('#tabla_xml_seguros tbody').on('click', 'td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = tabla_xml_seguros.row(tr);
            ...
            if( row.data().solicitudes == null || row.data().solicitudes == "null" ){
                $.post( general_base_url + "Pagos/carga_listado_factura" , { "idResidencial" : row.data().idResidencial, "id_usuario" : row.data().id_usuario } ).done( function( data ){
                    ...
                    if(data.length == 0){
                        ...
                        tabla_xml_seguros.row( tr ).data( row.data() );
                        row = tabla_xml_seguros.row( tr );
                        row.child( construir_subtablasSeguros( row.data().solicitudes ) ).show();
                        tr.addClass('shown');
    ...
    function construir_subtablasSeguros( data ){
    ...
    $("#tabla_xml_seguros tbody").on("click", ".regresar", function(e){
        e.preventDefault();
        ...
        $("#seeInformationModalPDFSeguros").modal();
        $("#seeInformationModalPDFSeguros .modal-body").append(`
        ...
        $("#seeInformationModalPDFSeguros .modal-body").append(`
            ...
            <div class="row d-flex align-center">
                <div class="col-sm-8 col-md-8 col-lg-8">
                    <div class="file-gph">
                        <input class="d-none" type="file" id="xmlfile2" onchange="changeNameSeguros(this)" name="xmlfile2"  accept="application/xml">
                ...
                <div class="col-sm-4 col-md-4 col-lg-4 p-0">
                    <button class="btn btn-azure w-90" type="button" onclick="xmlFuncionSeguros(${id_user})" id="cargar_xmlFuncion"><i class="fa fa-upload"></i> VERIFICAR Y <br> CARGAR</button>
                </div> 
            </div>`);
            
        $("#seeInformationModalPDFSeguros .modal-body").append('<b id="cantidadSeleccionadaMalSeguros"></b>');
        $("#seeInformationModalPDFSeguros .modal-body").append(`<div class="row">
        ...
        $("#seeInformationModalPDFSeguros .modal-footer").append(`
    ...
    $("#tabla_xml_seguros tbody").on("click", ".consultar_documentos_seguros", function(e){
        $('#spiner-loader').removeClass('hide');
        $("#modalAbrirFacturaSeguros .modal-body").html("");
        ...
        $("#modalAbrirFacturaSeguros").modal();
        $.getJSON( general_base_url + "Pagos/getDatosFactura/"+uuid+"/"+id_residencial).done( function( data ){
            $("#modalAbrirFacturaSeguros .modal-body").append('<div class="row">');
            ...
            if (!data.datos_solicitud['uuid'] == '' && !data.datos_solicitud['uuid'] == '0'){
                $.get(general_base_url+"Pagos/GetDescripcionXML/"+data.datos_solicitud['nombre_archivo']).done(function (dat) {
                    ...
                    $("#modalAbrirFacturaSeguros .modal-body").append(
                        ...
                        '</div>'+
                        '</div>');
                    $("#modalAbrirFacturaSeguros .modal-body").append('<div class ="row"><div class="col-md-12"><label style="font-size:14px; margin:0; color:gray;"><b>DESCRIPCIÓN</b></label><br><label style="font-size:12px; margin:0; color:gray;">'+descripcion+'</label><br><label style="font-size:12px; margin:0; color:gray;"> </label></div></div>');
                });
            }
            else {
                $("#modalAbrirFacturaSeguros .modal-body").append('<div class="col-md-12"><label style="font-size:16px; margin:0; color:black;">NO HAY DATOS A MOSTRAR</label></div>');
            }
            $("#modalAbrirFacturaSeguros .modal-body").append('</div>');
            $('#spiner-loader').addClass('hide');
        });
    });
}

function xmlFuncionSeguros(id_user) {
    subir_xml_seguros($("#xmlfile2"),id_user);
}

function subir_xml_seguros(input,id_user) {
    var data = new FormData();
    documento_xml = input[0].files[0];
    var xml = documento_xml;
    data.append("xmlfile", documento_xml);
    $.ajax({
        url: general_base_url + "SegurosComision/cargaxml2/"+id_user,
        data: data,
        cache: false,
        contentType: false,
        processData: false,
        ...
        success: function(data) {
            if (data.respuesta[0]) {
                ...
                cargar_info_xml_seguro(informacion_factura);
            }
            else {
                input.val('');
...
function cargar_info_xml_seguro(informacion_factura) {
    ...
    if((parseFloat(totalSeleccionado) + .10).toFixed(2) >= cantidadXml.toFixed(2) && cantidadXml.toFixed(2) >= (parseFloat(totalSeleccionado) - .10).toFixed(2)){
        var myCommentsList = document.getElementById('cantidadSeleccionadaMalSeguros');
...
$(document).on('click', '.verPDFSeguros', function () {
    var $itself = $(this);
    Shadowbox.open({
        content:    `<div>
