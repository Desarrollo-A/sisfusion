var tr;
var tabla_factura_seguros ;
var totaPago = 0;
let titulos_seguros = [];

$(document).ready(function() {
    $("#tabla_factura_seguros").prop("hidden", true);
    $.post(general_base_url+"Contratacion/lista_proyecto", function (data) {
        var len = data.length;
        for (var i = 0; i < len; i++) {
            var id = data[i]['idResidencial'];
            var name = data[i]['descripcion'];
            $("#proyectoFactura_seguros").append($('<option>').val(id).text(name.toUpperCase()));
        }
...
residencial = $('#proyectoFactura_seguros').val();
$("#condominioFactura_seguros").empty().selectpicker('refresh');
    $.ajax({
        ...
        success:function(response){
            ...
            for( var i = 0; i<len; i++)
            {
                ...
                $("#condominioFactura_seguros").append($('<option>').val(id).text(name));
            }
            $("#condominioFactura_seguros").selectpicker('refresh');
        }
    });
});

$('#proyectoFactura_seguros').change(function(){
    proyecto = $('#proyectoFactura_seguros').val();
    condominio = $('#condominioFactura_seguros').val();
...
$('#tabla_factura_seguros thead tr:eq(0) th').each(function (i) {
    if(i != 0){
        ...
        $( 'input', this ).on('keyup change', function () {
            if ($('#tabla_factura_seguros').DataTable().column(i).search() !== this.value ) {
                $('#tabla_factura_seguros').DataTable().column(i).search(this.value).draw();
            }
        });
    }else {
...
function getDataFacturaSeguros(proyecto, condominio){
        ...
        buttons: [{
            ...
            action: function() {
                if ($('input[name="idPagoFactura[]"]:checked').length > 0) {
                    ...
                    var idcomision = $(tabla_factura_seguros.$('input[name="idPagoFactura[]"]:checked')).map(function() {
                        return this.value;
                    }).get();
    ...
    $('#tabla_factura_seguros').on('draw.dt', function() {
        $('[data-toggle="tooltip"]').tooltip({ trigger: "hover" });
    });

    $("#tabla_factura_seguros tbody").on("click", ".consultar_logs_factura", function(e){
        e.preventDefault();
    ...
    $('#tabla_factura_seguros').on('click', 'input', function() {
        tr2 = $(this).closest('tr');
        var row = tabla_factura_seguros.row(tr2).data();
        if (row.monto == 0) {
...
$(document).on("click", ".checkPagosIndividual", function() {
    totaPago = 0;
    tabla_factura_seguros.$('input[type="checkbox"]').each(function () {
        let totalChecados = tabla_factura_seguros.$('input[type="checkbox"]:checked') ;
        let totalCheckbox = tabla_factura_seguros.$('input[type="checkbox"]');
        if(this.checked){
            tr = this.closest('tr');
            row = tabla_factura_seguros.row(tr).data();
