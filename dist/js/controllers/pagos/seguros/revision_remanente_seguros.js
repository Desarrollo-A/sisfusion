var tr;
var tabla_remanente_seguros ;
var totaPago = 0;
let titulos_remanente_seguro = [];

$(document).ready(function() {
    $("#tabla_remanente_seguros").prop("hidden", true);
    $.post(general_base_url+"Contratacion/lista_proyecto", function (data) {
        var len = data.length;
        for (var i = 0; i < len; i++) {
            var id = data[i]['idResidencial'];
            var name = data[i]['descripcion'];
            $("#proyectoRemanente_seguros").append($('<option>').val(id).text(name.toUpperCase()));
        }
...
residencial = $('#proyectoRemanente_seguros').val();
$("#condominioRemanente_seguros").empty().selectpicker('refresh');
    $.ajax({
        ...
        success:function(response){
            ...
            for( var i = 0; i<len; i++)
            {
                ...
                $("#condominioRemanente_seguros").append($('<option>').val(id).text(name));
            }
            $("#condominioRemanente_seguros").selectpicker('refresh');
        }
    });
});

$('#proyectoRemanente_seguros').change(function(){
    proyecto = $('#proyectoRemanente_seguros').val();
    condominio = $('#condominioRemanente_seguros').val();
...
$('#tabla_remanente_seguros thead tr:eq(0) th').each(function (i) {
    if(i != 0){
        var title = $(this).text();
        titulos_remanente_seguro.push(title);
        $(this).html('<input type="text" class="textoshead" data-toggle="tooltip" data-placement="top" title="' + title + '" placeholder="' + title + '"/>');
        $( 'input', this ).on('keyup change', function () {
            if ($('#tabla_remanente_seguros').DataTable().column(i).search() !== this.value ) {
                $('#tabla_remanente_seguros').DataTable().column(i).search(this.value).draw();
            }
        });
    }else {
...
function getDataRemanente_seguros(proyecto, condominio){
    ...
    $('#tabla_remanente_seguros').on('xhr.dt', function(e, settings, json, xhr) {
        ...
        document.getElementById("disponibleRemanente_seguros").textContent = to;
    });
    ...
    tabla_remanente_seguros = $("#tabla_remanente_seguros").DataTable({
        ...
        buttons: [{
                ...
                exportOptions: {
                    ...
                    format: {
                        header: function (d, columnIdx) {
                            return ' ' + titulos_remanente_seguro[columnIdx -1] + ' ';
                        }
                    }
                },
            },
            ...
            action: function() {
                if ($('input[name="idPagoRemanente[]"]:checked').length > 0) {
                    $('#spiner-loader').removeClass('hide');
                    var idcomision = $(tabla_remanente_seguros.$('input[name="idPagoRemanente[]"]:checked')).map(function() {
                        return this.value;
                    }).get();
        ...
        columnDefs: [{
            ...
            render: function (d, type, full, meta){
                if(full.estatus == 4){
                    if(full.id_comision){
                            return '<input type="checkbox" name="idPagoRemanente[]" class="checkPagosIndividual" style="width:20px;height:20px;"  value="' + full.id_pago_i + '">';
                    }else{
                        return '';
                    }
                }else{
    ...
    $('#tabla_remanente_seguros').on('draw.dt', function() {
        $('[data-toggle="tooltip"]').tooltip({
            trigger: "hover"
        });
    });

    $("#tabla_remanente_seguros tbody").on("click", ".consultar_logs_remanente", function(e){
        e.preventDefault();
    ...
    $('#tabla_remanente_seguros').on('click', 'input', function() {
        tr2 = $(this).closest('tr');
        var row = tabla_remanente_seguros.row(tr2).data();
        if (row.monto == 0) {
...
$(document).on("click", ".checkPagosIndividual", function() {
    totaPago = 0;
    tabla_remanente_seguros.$('input[type="checkbox"]').each(function () {
        let totalChecados = tabla_remanente_seguros.$('input[type="checkbox"]:checked') ;
        let totalCheckbox = tabla_remanente_seguros.$('input[type="checkbox"]');
        if(this.checked){
            tr = this.closest('tr');
            row = tabla_remanente_seguros.row(tr).data();
...
function selectAllRemanenteSeguros(e) {
    ...
    if(e.checked == true){
        ...
        $("#autorizarRemanente_seguros").html(formatMoney(numberTwoDecimal(tota2)));
    }
    if(e.checked == false){
        $(tabla_remanente_seguros.$('input[type="checkbox"]')).each(function (i, v) {
            if(v.checked == true){
