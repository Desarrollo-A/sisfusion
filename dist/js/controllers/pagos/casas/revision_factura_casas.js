...
$('#tabla_factura_casas thead tr:eq(0) th').each(function (i) {
    if(i != 0){
        ...
        $(this).html('<input id="all_facturas" type="checkbox" style="width:20px; height:20px;" onchange="selectAllFacturas(this)"/>');
    }
});
...
function getDataFacturaCasas(proyecto, condominio){
    ...
    tabla_factura_casas_2 = $("#tabla_factura_casas").DataTable({
        ...
        buttons: [{
            ...
            action: function() {
                if ($('input[name="idPagoFactura[]"]:checked').length > 0) {
                    ...
                    var idcomision = $(tabla_factura_casas_2.$('input[name="idPagoFactura[]"]:checked')).map(function() {
                        return this.value;
                    }).get();
        ...
        },
        {
            ...
            data: function( data ){
                ...
                const BTN_DETASI = `<button href="#" value="${data.id_pago_i}" data-value='"${data.lote}"' data-code="${data.cbbtton}" class="btn-data btn-blueMaderas consultar_logs_factura" title="Detalles"><i class="fas fa-info"></i></button>`;
        ...
        columnDefs: [{
            ...
            render: function (d, type, full, meta){
                if(full.estatus == 4){
                    if(full.id_comision){
                            return '<input type="checkbox" name="idPagoFactura[]" class="checkPagosIndividual_casas" style="width:20px;height:20px;"  value="' + full.id_pago_i + '">';
                    }else{
                        return '';
                    }
                }else{
    ...
    $('#tabla_factura_casas').on("click", 'input', function() {
        totaPago_casas = 0;
        tabla_factura_casas_2.$('input[type="checkbox"]').each(function () {
            let totalChecados = tabla_factura_casas_2.$('input[type="checkbox"]:checked') ;
            let totalCheckbox = tabla_factura_casas_2.$('input[type="checkbox"]');
            if(this.checked){
                trs = this.closest('tr');
                row = tabla_factura_casas_2.row(trs).data();
                totaPago_casas += parseFloat(row.impuesto); 
            }
            if( totalChecados.length == totalCheckbox.length ){
                $("#all_facturas").prop("checked", true);
            }else {
    ...
    $("#tabla_factura_casas tbody").on("click", ".cambiar_estatus_casas", function(){
        var tr1 = $(this).closest('tr');
        var row = tabla_factura_casas_2.row( tr1 );
        id_pago_i = $(this).val(); 
    ...
    $("#tabla_factura_casas tbody").on("click", ".regresar_estatus", function(){
        var tr1 = $(this).closest('tr');
        var row = tabla_factura_casas_2.row( tr1 );
        id_pago_i = $(this).val();
...
function selectAllFacturas(e) {
    tota2 = 0;
    if(e.checked == true){
        $(tabla_factura_casas_2.$('input[type="checkbox"]')).each(function (i, v) {
            trl = this.closest('tr');
            row = tabla_factura_casas_2.row(trl).data();
            tota2 += parseFloat(row.impuesto);
            if(v.checked == false){
                $(v).prop("checked", true);
            }
        }); 
        $("#autorizarFactura_casas").html(formatMoney(numberTwoDecimal(tota2)));
    }
    if(e.checked == false){
        $(tabla_factura_casas_2.$('input[type="checkbox"]')).each(function (i, v) {
            if(v.checked == true){
