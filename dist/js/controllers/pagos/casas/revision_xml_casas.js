...
$('#proyectoXml_casas').change(function(ruta){
    residencial = $('#proyectoXml_casas').val();
...
$('#tabla_xml_casas thead tr:eq(0) th').each( function (i) {
    ...
    $('input', this).on('keyup change', function() {
        if (tabla_xml_casas.column(i).search() !== this.value) {
            tabla_xml_casas
                .column(i)
                .search(this.value)
                .draw();
            var total = 0;
            var index = tabla_xml_casas.rows({
                selected: true,
                search: 'applied'
            }).indexes();
            var data = tabla_xml_casas.rows(index).data();
...
function getFacturaCommissions_casas(proyecto, condominio){
    ...
    tabla_xml_casas = $("#tabla_xml_casas").DataTable({
            ...
            exportOptions: {
                ...
                format: {
                    header: function (d, columnIdx) {
                        return ' ' + titulos_casas[columnIdx] + ' ';
                    }
                }
            },
        }],
    ...
    $('#tabla_xml_casas tbody').on('click', 'td.details-control', function () {
        ...
        var row = tabla_xml_casas.row(tr);
        if ( row.child.isShown() ) {
            ...
            if( row.data().solicitudes == null || row.data().solicitudes == "null" ){
                $.post( general_base_url + "Pagos/carga_listado_factura" , { "idResidencial" : row.data().idResidencial, "id_usuario" : row.data().id_usuario } ).done( function( data ){
                    ...
                    tabla_xml_casas.row( tr ).data( row.data() );
                    row = tabla_xml_casas.row( tr );
...
$(window).resize(function(){
    tabla_xml_casas.columns.adjust();
});
