...
$(document).ready(function() {
    $("#tabla_xml_casas").prop("hidden", true);
    $.post(general_base_url+"Contratacion/lista_proyecto", function (data) {
        var len = data.length;
        for (var i = 0; i < len; i++) {
            var id = data[i]['idResidencial'];
            var name = data[i]['descripcion'];
            $("#proyectoXml_casas").append($('<option>').val(id).text(name.toUpperCase()));
        }
        $("#proyectoXml_casas").selectpicker('refresh');
    }, 'json');
});

$('#proyectoXml_casas').change(function(){
    proyecto = $('#proyectoXml_casas').val();
    getDataXML_casas(proyecto);
});

$('#tabla_xml_casas thead tr:eq(0) th').each(function (i) {
    if(i != 0){
        var title = $(this).text();
        titulos_2.push(title);
        $(this).html('<input type="text" class="textoshead" data-toggle="tooltip" data-placement="top" title="' + title + '" placeholder="' + title + '"/>');
        $( 'input', this ).on('keyup change', function () {
            if ($('#tabla_xml_casas').DataTable().column(i).search() !== this.value ) {
                $('#tabla_xml_casas').DataTable().column(i).search(this.value).draw();
            }
        });
    }
});
...
function getDataXML_casas(proyecto){
    $('#tabla_xml_casas').on('xhr.dt', function(e, settings, json, xhr) {
        var total = 0;
    ...
    tabla_xml_2 = $("#tabla_xml_casas").DataTable({
    ...
    $('#tabla_xml_casas').on('draw.dt', function() {
    ...
    $("#tabla_xml_casas tbody").on("click", ".regresar", function(e){
        e.preventDefault();
    ...
    $("#tabla_xml_casas tbody").on("click", ".consultar_documentos", function(e){
        ...
        $.getJSON( general_base_url + "Pagos_casas/getDatosFactura/"+uuid+"/"+id_residencial).done( function( data ){
...
function subir_xml(input,id_user) {
    ...
    $.ajax({
        url: general_base_url + "Casas_comisiones/cargaxml2/"+id_user,
        data: data,
        cache: false,
        contentType: false,
        processData: false,
