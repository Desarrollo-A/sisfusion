...
$(document).ready(function() {
    ...
    $.post(general_base_url+"Contratacion/lista_proyecto", function (data) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            $("#proyectoRemanente").append($('<option>').val(id).text(name.toUpperCase()));
        }
        $("#proyectoRemanente").selectpicker('refresh');
    }, 'json');
});

$('#proyectoRemanente').change(function(){
residencial = $('#proyectoRemanente').val();
$("#condominioRemanente").empty().selectpicker('refresh');
    $.ajax({
        ...
        success:function(response){
            ...
            for( var i = 0; i<len; i++)
            {
                ...
                $("#condominioRemanente").append($('<option>').val(id).text(name));
            }
            $("#condominioRemanente").selectpicker('refresh');
        }
    });
});

$('#proyectoRemanente').change(function(){
    proyecto = $('#proyectoRemanente').val();
    condominio = $('#condominioRemanente').val();
...
$('#condominioRemanente').change(function(){
    proyecto = $('#proyectoRemanente').val();
    condominio = $('#condominioRemanente').val();
    if(condominio == '' || condominio == null || condominio == undefined){
        condominio = 0;
...
function getDataRemanente(proyecto, condominio){
    ...
    $('#tabla_remanente').on('xhr.dt', function(e, settings, json, xhr) {
        ...
        document.getElementById("disponibleRemanente").textContent = to;
    });
        ...
        buttons: [{
            ...
            action: function() {
                if ($('input[name="idPagoRemanente[]"]:checked').length > 0) {
                    ...
                    var idcomision = $(tabla_remanente.$('input[name="idPagoRemanente[]"]:checked')).map(function() {
                        return this.value;
                    }).get();
    ...
    $('#tabla_remanente').on('click', 'input', function() {
        tr2 = $(this).closest('tr');
        var row = tabla_remanente.row(tr2).data();
        if (row.monto == 0) {
        ...
        $("#autorizarRemanente").html(formatMoney(numberTwoDecimal(totaPago)));
    });
...
function selectAll(e) {
    ...
    if(e.checked == true){
        ...
        $("#autorizarRemanente").html(formatMoney(numberTwoDecimal(tota2)));
    }
    if(e.checked == false){
        $(tabla_remanente.$('input[type="checkbox"]')).each(function (i, v) {
            if(v.checked == true){
