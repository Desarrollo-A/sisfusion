...
$("#file-uploadE").on('change', function(e){ 
    $("#table_datos2").removeClass('hide');
    $("#savingProspect").removeClass('hide');
...
document.getElementById('file-uploadE').addEventListener('change', readFile, false);
let contadorSede = 0;
let contadorSede2 =0;
let ContadorErr= 0;
let ContadorFila=2;
...
$(document).ready(function() {
    $('#table_datos2').DataTable( {
        ...
        columns: [
            { data: lines.id_sede, render: function(data){ //alert(data); // let v;
...
let titulos_intxt = [];
$('#table_datos2 thead tr:eq(0) th').each(function (i) {
    var title = $(this).text();
    titulos_intxt.push(title);
    $(this).html(`<input type="text" class="textoshead" data-toggle="tooltip" data-placement="top" title="${title}" placeholder="${title}"/>`);
    $('input', this).on('keyup change', function () {
        if ($('#table_datos2').DataTable().column(i).search() !== this.value)
            $('#table_datos2').DataTable().column(i).search(this.value).draw();
    });
});
