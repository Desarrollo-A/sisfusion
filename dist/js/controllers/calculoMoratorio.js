			...
			if($scope.infoMoratorio.plazo >= 0 && $scope.infoMoratorio.plazo <=36)
			{
				...
				$scope.dtColumns = [
					...
					DTColumnBuilder.newColumn('interes').withTitle('Intereses').renderWith(function (data, type, full)
					{
						...
						{
							if(full['deudaOrdinario'] <= 0)//si no hay nada en el adeudo
							{
								showDescInt = '<br><span style="color:red;font-size: 0.8em"><i>Se liquidó el interes: '+full['deudaOrdinario'].toLocaleString('es-MX', {style: 'currency', currency: 'MXN'})+'</i></span>';
							}
							else
							{
