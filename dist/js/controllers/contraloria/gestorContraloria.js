...
let idopcion_modelo = "";
let idLote = "";
let usuariosPermitidosRL = [
  2815, 2875, 12276, 2767, 11947, 2807, 9775, 14342, 2749, 11815,
];
let usuariosPermitidosIntercambio = [
  5342, 2767, 11947, 2807, 9775, 14342, 2749, 11815,
];
...
$(document).ready(function () {
  $(
    "#divTablaRL, #divTablaIntercambio, #divTablaCambioRL, #divmodelosTable"
  ).addClass("hide");
  $.getJSON("getOpcionesPorCatalogo").done(function (data) {
    for (let i = 0; i < data.length; i++) {
      if (data[i]["id_catalogo"] == 155) {
        if (
          data[i]["id_opcion"] == 1 &&
          usuariosPermitidosRL.includes(id_usuario_general)
        ) {
          $("#selector").append(
          ...
          data[i]["id_opcion"] == 2 &&
          usuariosPermitidosIntercambio.includes(id_usuario_general)
        ) {
          $("#selector").append(
            $("<option>").val(data[i]["id_opcion"]).text(data[i]["nombre"])
          );
          ...
          data[i]["id_opcion"] == 3 &&
          usuariosPermitidosModelosCasas.includes(id_usuario_general)
        ) {
          $("#selector").append(
            $("<option>").val(data[i]["id_opcion"]).text(data[i]["nombre"])
          );
...
function loadSelectOptions() {
  $.post(
    `${general_base_url}Contratacion/lista_proyecto`,
    function (data) {
      for (var i = 0; i < data.length; i++) {
        $("#selectProyecto").append(
          $("<option>")
            .val(data[i]["idResidencial"])
  ...
  $.post(
    `${general_base_url}Contratacion/lista_estatus`,
    function (data) {
      for (var i = 0; i < data.length; i++) {
        $("#idEstatus").append(
          $("<option>").val(data[i]["idStatusLote"]).text(data[i]["nombre"])
        );
      }
  ...
  $.post(
    `${general_base_url}OperacionesPorCatalogo/listacatalogo`,
    function (data) {
      for (var i = 0; i < data.length; i++) {
        $("#cambiarrepresentante").append(
          $("<option>").val(data[i]["id_opcion"]).text(data[i]["nombre"])
        );
      }
...
$("#selectProyecto").change(function () {
  ...
  $(document).ready(function () {
    $.post(
      `${general_base_url}Contratacion/lista_condominio/${index_idResidencial}`,
      function (data) {
        for (var i = 0; i < data.length; i++) {
          $("#selectCondominio").append(
            $("<option>").val(data[i]["idCondominio"]).text(data[i]["nombre"])
          );
        }
...
function ConstruirTablaCAmbiarRepresentante(idCondominio) {
  ...
  if (idCondominio) {
    $("#tablaCambiarRepresentanteLegal thead tr:eq(0) th").each(function (i) {
      ...
      $(this).html(
        `<input type="text" class="textoshead" data-toggle="tooltip" data-placement="top" title="${title}" placeholder="${title}"/>`
      );
    ...
    tablaCambiarRepresentanteLegal = $(
      "#tablaCambiarRepresentanteLegal"
    ).DataTable({
      dom:
        "Brt" +
      ...
      columns: [
        ...
        },
        {
          orderable: false,
          data: function (d) {
            return `<div class="d-flex justify-center"><button href="#" class="btn-data btn-blueMaderas modalCambioRepresentanteB" data-nombreLote="${
              d.nombreLote
            }" data-idlote="${d.idLote}" data-cliente="${
              d.idCliente
            }" data-idrl="${
              d.id_rl
            }" data-toggle="tooltip" data-placement="top" title="${_(
              "editar"
        ...
        data: function (d) {
          return `
                        <div class="d-flex justify-center">
                            <button href="#" id="btnEditarTipoVenta" class="btn-data btn-blueMaderas" data-idlote="${d.idLote}" title="EDITAR INFORMACIÓN">
  ...
  $("#btnConfirmarCambioTipoVenta")
    ...
    .on("click", function () {
      if (tipoVenta && idLot) {
        ...
        console.error(
          "No se han seleccionado valores válidos para la actualización"
        );
      }
    });
  ...
  tablaGestor = $("#gestorContraloria").DataTable({
    ...
    columns: [
      ...
      { data: "fecha_creacion" },
      {
        orderable: false,
        data: function (d) {
          return `<div class="d-flex justify-center"><button href="#" class="btn-data agregar ${
            d.estatus == 0 ? "btn-green" : "btn-warning"
          }" data-nombre="${
            d.nombre
          }" data-toggle="tooltip" data-placement="top" title="EDITAR INFORMACIÓN" data-transaccion="${
            d.estatus == 0 ? "1" : "2"
          }" data-idopcion="${d.id_opcion}"><i class="${
            d.estatus == 0 ? "fas fa-unlock" : "fas fa-lock"
          }"></i></button></div>`;
        },
      },
    ],
...
function llenarTablaCambioRL(tipoOperacion) {
  if ($("#nombreLote").val().length == 0) {
    alerts.showNotification(
      "top",
      "right",
      "Asegúrate de ingresar un valor.",
      "warning"
    );
  } else {
...
$(document).on("click", ".agregar", function () {
  ...
  document.getElementById("titulo").innerHTML =
    tipoT == 0 ? `Agregar Representante Legal` : `Editar Representante Legal`;
  document.getElementById("btnAgregar").innerHTML =
    tipoT == 0 ? `Agregar` : `Actualizar`;
  if (tipoT == 0) {
    ...
    document.getElementById("confirmacion").innerHTML =
      tipoT == 1
        ? `¿Estás seguro de activar al Representante Legal <b>${$(this).attr(
            "data-nombre"
          )}</b>?`
        : `¿Estás seguro de desactivar al Representante Legal <b>${$(this).attr(
            "data-nombre"
          )}</b>?`;
  }
      ...
      success: function (data) {
        if (data) {
          alerts.showNotification(
            ...
            "right",
            `Registro ${
              tipoTransaccion == 0 ? "insertado" : "actualizado"
            } exitosamente.`,
            "success"
          );
...
$(document).on("click", ".modalCambioRepresentanteB", function () {
  ...
  ).innerHTML = `¿Estás seguro de realizar el cambio de estatus del lote <b>${$(
    this
  ).attr("data-nombreLote")}</b>?`;
...
$(document).on("click", "#btnActualizarRL", function (e) {
  ...
  if (validarOpcionNombre == 0) {
    alerts.showNotification(
      "top",
      "right",
      "Asegúrate de seleccionar una opción",
      "warning"
    );
  } else {
...
$(document).on("click", "#btnCambiarRL", function (e) {
  ...
  if (validarOpcionRl == 0) {
    alerts.showNotification(
      "top",
      "right",
      "Asegúrate de seleccionar un representante legal",
      "warning"
    );
  } else {
    if (selectedRl !== 0) {
        ...
        success: function (data) {
          ...
          if (data.message == "OK") {
            ...
            alerts.showNotification(
              "top",
              "right",
              "Ha ocurrido un error al avanzar" +
                " el registro, intentalo nuevamente",
              "danger"
            );
          }
        },
...
$(document).ready(function () {
  ...
  $("#modelosTable").DataTable({
    ...
    columns: [
      ...
      },
      {
        orderable: false,
        data: function (d) {
          return `<div class="d-flex justify-center"><button class="btn-data agregarCasa ${
            d.estatus == 2 ? "btn-green" : "btn-warning"
          }" data-toggle="tooltip" data-placement="top" title="EDITAR ESTATUS" data-transaccionmodelo="${
            d.estatus == 2 ? "1" : "2"
          }" data-modelo="${d.modelo}" data-idopcion_modelo="${
            d.idModelo
          }" data-estatus="${d.estatus}"><i class="${
            d.estatus == 2 ? "fas fa-unlock" : "fas fa-lock"
          }"></i></button> </div>`;
        },
      },
    ],
  ...
  $(document).on("click", ".agregarCasa", function () {
    ...
    $("#titulomodelo").text(
      tipoTm == 0 ? "Agregar modelo de casa" : "Actualizar estado"
    );
        ...
        success: function (response) {
          ...
          if (data.success) {
            alerts.showNotification(
              ...
              "right",
              `Registro ${
                tipoTransaccionModelo == 0 ? "insertado" : "actualizado"
              } exitosamente.`,
              "success"
            );
          } else {
