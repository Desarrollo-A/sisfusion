    ...
    $.getJSON("getUserInformation/"+id_usuario).done( function( data ){
        $.each( data, function(i, v){
            ...
            if(v.id_rol == 2){
                ...
                document.getElementById('multirol').innerHTML = '';
                console.log(data)
                datosMulti = data[0].multirol;
                lengthmultirol = datosMulti.length;
                console.log(datosMulti)
                $('#btnmultirol').append(`
                <button class="btn-data btn-green" type="button" id="btnMultiRol" data-toggle="tooltip" data-placement="top" title="Agregar rol"><i class="fas fa-user-plus"></i></button>
                `);
                $('#index').val(lengthmultirol);
                for (let m = 0; m < lengthmultirol; m++) {
                  $('#multirol').append(`
                  <div class="col-md-12 aligned-row" id="mult_${m}">
                  <div class="col-md-6 pr-0 pr-0">
                      <div class="form-group text-left m-0">
                      <input id="idRU_${m}" value="${datosMulti[m].idRU}" name="idRU_${m}" class="form-control input-gral" type="hidden"> 
                          <label class="control-label">Tipo de miembro (<small class="isRequired">*</small>)</label>
                          <select class="selectpicker select-gral m-0" required name="multi_${m}" id="multi_${m}" data-style="btn"
                          data-show-subtext="true"
                          title="Selecciona una opción"
                          data-size="7"
                          data-live-search="true" data-container="body"
                  ...
                  <div class="col-md-4 pr-0 pr-0">
                      <div class="form-group text-left m-0">
                          <label class="control-label">Sede (<small class="isRequired">*</small>)</label>
                          <select class="selectpicker select-gral m-0" required onchange="validarSede(${m});" name="sedes_${m}" id="sedes_${m}" data-style="btn"
                          data-show-subtext="true"
                          title="Selecciona una opción"
                          data-size="7"
                          data-live-search="true" data-container="body"
                  ...
                  <div class="col-md-2 justify-center d-flex align-end">
                    <div class="form-group m-0 p-0">
                          <button class="btn-data btn-warning mb-1" type="button" onclick="borrarMulti(${m},${datosMulti[m].idRU})" data-toggle="tooltip" data-placement="top" title="Eliminar rol"><i class="fa fa-trash"></i></button>
                      </div>
              </div>`);
            ...
            for (var i = 0; i < puestos.length; i++) {
                var id = puestos[i].id;
                var name = puestos[i].nombre;
                $(`#multi_${m}`).append($('<option>').val(id).text(name.toUpperCase()));
            }
            for (var i = 0; i < sedes.length; i++) {
                var id = sedes[i].id;
                var name = sedes[i].nombre;
                $(`#sedes_${m}`).append($('<option>').val(id).text(name.toUpperCase()));
            }
    
            $(`#multi_${m}`).selectpicker();
            $(`#multi_${m}`).val(datosMulti[m].idRol);
            $(`select[name=multi_${m}]`).change();
            $(`#sedes_${m}`).selectpicker();
            $(`#sedes_${m}`).val(datosMulti[m].idSede);
            $(`select[name=sedes_${m}]`).change();
                }
            }else{
                document.getElementById('btnmultirol').innerHTML = '';
                document.getElementById('multirol').innerHTML = '';
