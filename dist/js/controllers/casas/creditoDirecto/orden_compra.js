    ...
    { data: function(data)
        {
            ...
            if(data.documento == null && data.proyectos == 0){
                return '<div class="d-flex justify-center">' + upload_button + '</div>'
            }
            else if (data.documento != null && data.proyectos == 0){
...
function filter (data) {
    ...
    return JSON.stringify(arrayData.filter(row => row.proyectos == 0))
}
...
//     $.ajax({
//         type: 'POST',
//         url: `${general_base_url}casas/returnFlagsPaso2`,
//         data: data,
//         contentType: false,
//         processData: false,
//         success: function (response) {
...
select_lote = function(data){ // funcion para el avance del lote
    let form = new Form({
        ...
        onSubmit: function(data){
            ...
            $.ajax({
                type: 'POST',
                url: `${general_base_url}casasDirecto/avanceOrdenCompra`,
                data: data,
                contentType: false,
                processData: false,
                success: function (response) {
        ...
        fields: [
            ...
            new HiddenField({ id: 'idProceso', value: data.idProceso }),
            new HiddenField({ id: 'proceso', value: data.proceso }),
            new HiddenField({ id: 'proyectos', value: 1 }),
