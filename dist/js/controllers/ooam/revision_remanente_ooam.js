...
$(document).ready(function() {
    ...
    $.post(general_base_url+"Contratacion/lista_proyecto", function (data) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            $("#proyectoRemanenteOOAM").append($('<option>').val(id).text(name.toUpperCase()));
        }
        $("#proyectoRemanenteOOAM").selectpicker('refresh');
    }, 'json');
});

$('#proyectoRemanenteOOAM').change(function(){
residencial = $('#proyectoRemanenteOOAM').val();
$("#condominioRemanenteOOAM").empty().selectpicker('refresh');
    $.ajax({
        ...
        success:function(response){
            ...
            for( var i = 0; i<len; i++)
            {
                ...
                $("#condominioRemanenteOOAM").append($('<option>').val(id).text(name));
            }
            $("#condominioRemanenteOOAM").selectpicker('refresh');
        }
    });
});

$('#proyectoRemanenteOOAM').change(function(){
    proyecto = $('#proyectoRemanenteOOAM').val();
    condominio = $('#condominioRemanenteOOAM').val();
    if(condominio == '' || condominio == null || condominio == undefined){
        condominio = 0;
    }
    getDataRemanenteOOAM(proyecto, condominio);
});

$('#condominioRemanenteOOAM').change(function(){
    proyecto = $('#proyectoRemanenteOOAM').val();
    condominio = $('#condominioRemanenteOOAM').val();
    if(condominio == '' || condominio == null || condominio == undefined){
        condominio = 0;
...
function getDataRemanenteOOAM(proyecto, condominio){
    ...
    $('#tabla_remanente_ooam').on('xhr.dt', function(e, settings, json, xhr) {
        ...
        document.getElementById("disponibleRemanenteOOAM").textContent = to;
    });
    ...
    tabla_remanente_ooam = $("#tabla_remanente_ooam").DataTable({
        ...
        buttons: [{
            ...
            action: function() {
                if ($('input[name="idPagoRemanenteOOAM[]"]:checked').length > 0) {
                    $('#spiner-loader').removeClass('hide');
                    var idcomision = $(tabla_remanente_ooam.$('input[name="idPagoRemanenteOOAM[]"]:checked')).map(function() {
                        return this.value;
                    }).get();
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if(data == 1) {
                                ...
                                $("#autorizarRemanenteOOAM").html(formatMoney(0));
                                    ...
                                    <center>
                                        <img style='width: 75%; height: 75%;' src="${general_base_url}dist/img/send_intmex.gif" >
                                            <p style='color:#676767;'>Comisiones de esquema 
                                                <b>remanente</b>, fueron enviadas a 
        ...
        columnDefs: [{
            ...
            render: function (d, type, full, meta){
                if(full.estatus == 4){
                    if(full.id_comision){
                            return '<input type="checkbox" name="idPagoRemanenteOOAM[]" class="checkPagosIndividualOOAM" style="width:20px;height:20px;"  value="' + full.id_pago_i + '">';
                    }else{
                        return '';
                    }
                }else{
    ...
    $('#tabla_remanente_ooam').on('draw.dt', function() {
    ...
    $('#tabla_remanente_ooam').on('click', 'input', function() {
        tr2 = $(this).closest('tr');
        var row = tabla_remanente_ooam.row(tr2).data();
        if (row.montoOOAM == 0) {
        ...
        $("#autorizarRemanenteOOAM").html(formatMoney(numberTwoDecimal(totaPagoOOAM)));
    });
    ...
    $("#tabla_remanente_ooam tbody").on("click", ".cambiar_estatus", function(){
        ...
        id_pago_i = $(this).val();
        $("#modalPausarRemanenteOOAM .modal-body").html("");
        $("#modalPausarRemanenteOOAM .modal-body").append(
            '<div class="row">'+
                '<div class="col-lg-12">'+
        ...
        $("#modalPausarRemanenteOOAM").modal();
    });
}

$("#formPausarRemanenteOOAM").submit( function(e) {
    e.preventDefault();
}).validate({
    submitHandler: function( form ) {
        ...
        $.ajax({
            ...
            success: function(data){
                if( data[0] ){
                    $("#modalPausarRemanenteOOAM").modal('toggle' );
...
function selectAllOOAM(e) {
    ...
    if(e.checked == true){
        ...
        $("#autorizarRemanenteOOAM").html(formatMoney(numberTwoDecimal(tota2)));
    }
    if(e.checked == false){
        $(tabla_remanente_ooam.$('input[type="checkbox"]')).each(function (i, v) {
            if(v.checked == true){
