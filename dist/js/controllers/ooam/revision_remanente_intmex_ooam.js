...
$(document).ready(function () { 
    ...
    $.post(general_base_url + "Pagos/lista_roles", function (data) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            $("#puesto_ooam").append($('<option>').val(id).text(name.toUpperCase()));
        }
...
$('#puesto_ooam').change(function (ruta) {
    rol = $('#puesto_ooam').val();
    $("#usuario_ooam").empty().selectpicker('refresh');
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                $("#usuario_ooam").append($('<option>').val(id).text(name));
            }
            if (len <= 0) {
                $("#usuario_ooam").append('<option selected="selected" disabled>NO HAY OPCIONES</option>');
            }
            $("#usuario_ooam").selectpicker('refresh');
        }
    });
});

$('#usuario_ooam').change(function (ruta) {
    proyecto = $('#puesto_ooam').val();
    condominio = $('#usuario_ooam').val();
...
function getAssimilatedCommissionsOoam(proyecto, condominio) {
    ...
    $("#tabla_remanente_ooam tbody").on("click", ".cambiar_estatus", function () {
        ...
        var row = tabla_remanente_ooam.row(trs);
        id_pago_i = $(this).val();
    ...
    $("#tabla_remanente_ooam tbody").on("click", ".regresar_estatus", function () {
        var trs = $(this).closest('trs');
        var row = tabla_remanente_ooam.row(trs);
        id_pago_i = $(this).val();
...
$(window).resize(function () {
    tabla_remanente_ooam.columns.adjust();
});
