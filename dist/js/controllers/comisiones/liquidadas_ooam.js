$(document).ready(function () {
    $('#tabla_comisiones_liquidadas_ooam').on('draw.dt', function() {
    ...
    let titulos_intxt = [];
    $('#tabla_comisiones_liquidadas_ooam thead tr:eq(0) th').each( function (i) {
        ...
        if (i != 0 ) {
            ...
            $( 'input', this ).on('keyup change', function () {
                if ($('#tabla_comisiones_liquidadas_ooam').DataTable().column(i).search() !== this.value ) {
                    $('#tabla_comisiones_liquidadas_ooam').DataTable().column(i).search(this.value).draw();
                }
                var index = $('#tabla_comisiones_liquidadas_ooam').DataTable().rows({
                    selected: true,
                    search: 'applied'
                }).indexes();
            });
    ...
    liquidadasDataTableOOAM = $('#tabla_comisiones_liquidadas_ooam').dataTable({
        ...
        ajax: {
            url: general_base_url + 'Ooam/getDataLiquidadasPagoOOAM',
            type: "POST",
            cache: false,
            data: function( d ){}
        }
    ...
    $('#tabla_comisiones_liquidadas_ooam tbody').on('click', 'td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = $('#tabla_comisiones_liquidadas_ooam').DataTable().row(tr);
    ...
    $("#tabla_comisiones_liquidadas_ooam tbody").on("click", ".verify_neodata", async function(){ 
...
function showDetailModal(idPlan) {
    ...
    $.ajax({
        url: `${general_base_url}Ooam/getDetallePlanesComisionesOOAM/${idPlan}`,
        type: 'GET',
        dataType: 'json',
