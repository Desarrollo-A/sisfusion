...
function arrancarTabla() {
    $('#tabla_dispersion_casas thead tr:eq(0) th').each(function (i) {
        $(this).css('text-align', 'center');
        var title_casa = $(this).text();
        titulos_intxt_dispersion.push(title_casas);
        if (i != 0 ) {
            $(this).html(`<input data-toggle="tooltip" data-placement="top" placeholder="${title_casa}" title="${title_casa}"/>` );
    ...
    dispersionDataTable_casas = $('#tabla_dispersar_comisiones').dataTable({
            ...
            exportOptions: {
                ...
                format: {
                    header: function (d, columnIdx) {
                        return ' ' + titulos_intxt_dispersion[columnIdx] + ' ';
                    }
                }
            }
        }],
                ...
                if(d.penalizacion == 1 && d.bandera_penalizacion == 0 && d.id_porcentaje_penalizacion != '4') {
                    ...
                    if((d.totalNeto2==null || d.totalNeto2==''|| d.totalNeto2==0) && d.proceso == 0) {
                        ...
                        if(disparador != 0){
                            ...
                            class = "btn-data ${varColor} verify_neodata" 
                            data-toggle="tooltip" ${disableRew}  data-placement="top" title="${ Mensaje }">
                            <span class="material-icons">verified_user</span></button> ${RegresaActiva}`;
                        ...
                        if(data == true){
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    $('#tabla_dispersion_casas').DataTable().ajax.reload(null, false );
...
function numerosDispersion_table(){
    $('#monto_label').html('');
    $('#pagos_label').html('');
    $('#lotes_label').html('');
