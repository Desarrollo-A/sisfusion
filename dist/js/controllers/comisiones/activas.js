$(document).ready(function () {
    ...
    $('#tabla_comisiones_activas thead tr:eq(0) th').each( function (i) {
        ...
        if (i != 0 ) {
            ...
            $( 'input', this ).on('keyup change', function () {
                ...
                var index = $('#tabla_comisiones_activas').DataTable().rows({ selected: true, search: 'applied' }).indexes();
    ...
    $('#tabla_comisiones_activas').on('draw.dt', function() {
...
$("#actualizarBanderaForm").on('submit', function(e){
    e.preventDefault();
    $.ajax({
        type: 'POST',
        url: 'updateBandera',
        ...
        success: function(data) {
            if (data == 1) {
                $('#actualizarBanderaModal').modal("hide");
                $("#id_pagoc").val("");
...
$(document).on('click', '.update_bandera', function(e){
    ...
    nombreLote = $(this).attr("data-nombreLote");
    $("#actualizarBanderaModal .modal-body").html('');
    $("#actualizarBanderaModal .modal-header").html('');
    $("#actualizarBanderaModal .modal-header").append('<h4 class="modal-title">Enviar a dispersion de comisiones: <b>'+nombreLote+'</b></h4>');
    $("#actualizarBanderaModal .modal-body").append('<input type="hidden" name="id_pagoc" id="id_pagoc"><input type="hidden" name="param" id="param">');
    $("#actualizarBanderaModal").modal();
