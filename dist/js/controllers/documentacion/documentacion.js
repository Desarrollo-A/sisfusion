...
$("#idCondominio").change(function () {
  ...
  $("#spiner-loader").removeClass("hide");
  $.ajax({
    url: `${general_base_url}registroCliente/getLotesAsesor/${$(
      this
    ).val()}/${$("#idResidencial").val()}`,
    type: "post",
    dataType: "json",
    success: function (response) {
      for (let i = 0; i < response.length; i++) {
        $("#idLote").append(
          $("<option>")
            .val(response[i]["idLote"] + "," + response[i]["venta_compartida"])
            .text(response[i]["nombreLote"])
        );
      }
      ...
      success: function (response) {
        if (response.length > 0) {
          for (var i = 0; i < response.length; i++) {
            let labelStatus = "";
            if (response[i]["status"] == 1) labelStatus = " [ACTIVO]";
            $("#idCliente").append(
              $("<option>")
                .val(response[i]["id_cliente"])
                .text(response[i]["nombreCliente"] + labelStatus)
            );
          }
        } else {
          $("#idCliente").append(
            $("<option selected>").val(0).text("SIN CLIENTES")
          );
        }
...
function cargarTabla(idLote, idCliente = "") {
  ...
  $("#tableDoct thead tr:eq(0) th").each(function (i) {
    ...
    titulos.push(title);
    $(this).html(
      '<input type="text" data-toggle="tooltip" data-placement="top" title="' +
        title +
        '" class="textoshead"  placeholder="' +
        title +
        '"/>'
    );
        ...
        render: function (data) {
          ...
          if (
            ...
            (includesArray(rolesPermitidosContratoEspecial, id_rol_general) ||
              includesArray(
                usuariosPermitidosContratoEspecial,
                id_usuario_general
              ))
          ) {
          ...
          if (
            data.tipo_doc == TipoDoc.CONTRATO ||
            data.tipo_doc == TipoDoc.RESCISION ||
            data.tipo_doc == TipoDoc.ADDENDUM
          ) {
            // CONTRATO
            if (data.expediente == null || data.expediente === "") {
              // NO HAY DOCUMENTO CARGADO
              buttonMain =
                includesArray(
                  movimientosPermitidosEstatus7,
                  data.idMovimiento
                ) && includesArray(rolesPermitidosEstatus7, id_rol_general)
            ...
            buttonMain = crearBotonAccion(AccionDoc.DOC_CARGADO, data); // SE VE A MONSTRAR ENABLED EL BOTÓN PARA VER EL ARCHIVO
            if (
              includesArray(movimientosPermitidosEstatus7, data.idMovimiento) &&
              includesArray(rolesPermitidosEstatus7, id_rol_general)
            ) {
              buttonDelete = crearBotonAccion(AccionDoc.ELIMINAR_DOC, data);
            }
          ...
          if (data.tipo_doc == TipoDoc.CORRIDA) {
            // CORRIDA
            if (data.expediente == null || data.expediente === "") {
              // NO HAY DOCUMENTO CARGADO
              buttonMain =
                includesArray(
                  movimientosPermitidosEstatus6,
                  data.idMovimiento
                ) && includesArray(rolesPermitidosEstatus6And15, id_rol_general)
            ...
            if (
              includesArray(movimientosPermitidosEstatus6, data.idMovimiento) &&
              includesArray(rolesPermitidosEstatus6And15, id_rol_general)
            ) {
              buttonDelete = crearBotonAccion(AccionDoc.ELIMINAR_DOC, data);
            }
          ...
          if (
            ...
            if (data.expediente == null || data.expediente === "") {
              ...
              buttonMain =
                includesArray(
                  movimientosPermitidosContratoFirmado,
                  data.idMovimiento
                ) && includesArray(rolesPermitidosEstatus6And15, id_rol_general)
          ...
          if (
            ...
            // EXISTE EL DEPÓSITO DE SERIEDAD (VERSIÓN NUVEA)
            buttonMain = crearBotonAccion(AccionDoc.DOC_CARGADO, data); // SE VE A MONSTRAR ENABLED EL BOTÓN PARA VER EL ARCHIVO
            return `<div class="d-flex justify-center">${buttonMain}</div>`;
          }
          if (
            data.tipo_doc === TipoDoc.DS_NEW &&
            data.expediente === "Depósito de seriedad versión anterior"
          ) {
            // EXISTE EL DEPÓSITO DE SERIEDAD (VERSIÓN VIEJITA)
            buttonMain = crearBotonAccion(AccionDoc.DOC_CARGADO, data); // SE VE A MONSTRAR ENABLED EL BOTÓN PARA VER EL ARCHIVO
            return `<div class="d-flex justify-center">${buttonMain}</div>`;
          }
          if (data.tipo_doc == TipoDoc.EVIDENCIA_MKTD_OLD) {
            // EXISTE LA RAMA CON LA EVIDENCIA DE MKTD (OLD)
            buttonMain = crearBotonAccion(AccionDoc.DOC_CARGADO, data); // SE VE A MONSTRAR ENABLED EL BOTÓN PARA VER EL ARCHIVO
            return `<div class="d-flex justify-center">${buttonMain}</div>`;
          }
          if (data.tipo_doc === TipoDoc.AUTORIZACIONES) {
            // EXISTE LA RAMA DE AUTORIZACIONES
            buttonMain = crearBotonAccion(AccionDoc.DOC_CARGADO, data); // SE VE A MONSTRAR ENABLED EL BOTÓN PARA VER EL ARCHIVO
            return `<div class="d-flex justify-center">${buttonMain}</div>`;
          }
          if (data.tipo_doc === TipoDoc.PROSPECTO) {
            // EXISTE LA RAMA DEL PROSPECTO
            buttonMain = crearBotonAccion(AccionDoc.DOC_CARGADO, data); // SE VE A MONSTRAR ENABLED EL BOTÓN PARA VER EL ARCHIVO
            return `<div class="d-flex justify-center">${buttonMain}</div>`;
          }
...
$(document).on("click", ".verDocumento", function () {
  ...
  if (
    $itself.attr("data-tipoDocumento") === TipoDoc.DS_NEW ||
    $itself.attr("data-tipoDocumento") === TipoDoc.DS_OLD
  ) {
    const idCliente = $itself.attr("data-idCliente");
    const urlDs =
      $itself.attr("data-expediente") === "Depósito de seriedad"
        ? "deposito_seriedad"
        : "deposito_seriedad_ds";
  ...
  if ($itself.attr("data-tipoDocumento") === TipoDoc.PROSPECTO) {
    const urlProspecto =
      $itself.attr("data-lp") == 6
        ? "printProspectInfoMktd"
        : "printProspectInfo";
    pathUrl =
      `${general_base_url}clientes/${urlProspecto}/` +
      $itself.attr("data-idProspeccion");
  }
...
function abrirModalAutorizaciones(idLote) {
  $.post(
    `${general_base_url}registroLote/get_auts_by_lote/${idLote}`,
    function (data) {
      let statusProceso;
      $("#auts-loads").empty();
      $.each(JSON.parse(data), function (i, item) {
        if (item["estatus"] == 0) {
          ...
          statusProceso =
            "<small class='label lbl-orangeYellow'>En proceso</small>";
