var noFiles = _('no-files-available');

$(document).ready(function () {
    onLoadTranslations(function(){
        document.getElementById('test').title = _files;
        
    });

    onChangeTranslations(function(){
        $("#test").attr("title", "si"); // revisar
    });
        ...
        $.post(`${general_base_url}index.php/asesor/getAllFoldersPDF`,  function(data) {
            ...
            if(data.length < 0){
                ...
                $('#msg').append('<center><h2 style="color: #a0a0a0;font-weight: 100" data-i18n="'+noFiles+'">No hay Carpetas disponibles</h2></center>');
            }
        }, 'json');    
});
